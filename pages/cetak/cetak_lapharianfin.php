  ...
  <tbody>
    <tr>
	  ...
	  <table class="table table-bordered table-hover table-striped nowrap" id="examplex" style="width:100%" border="1">
        <thead class="bg-blue">
          <tr>
            <th bgcolor="#72EF71"><div align="center">PROSES</div></th>
            <th bgcolor="#72EF71"><div align="center">Operation</div></th>
            ...
            <th bgcolor="#72EF71"><div align="center">Lot</div></th>
            <th bgcolor="#72EF71"><div align="center">Keluar Basah</div></th>
            <th bgcolor="#72EF71"><div align="center">Lot</div></th>
            <th bgcolor="#72EF71"><div align="center">Keluar Kering</div></th>
            <th bgcolor="#72EF71"><div align="center">Lot</div></th>
...
$sqlsB_oven_k  = sqlsrv_query($conS, "SELECT
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN a.qty ELSE 0 END) AS basah,
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN 1 ELSE 0 END) AS basah_lot,
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN a.panjang ELSE 0 END) AS yard,
  SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN a.qty ELSE 0 END) AS kering,
  SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN 1 ELSE 0 END) AS kering_lot,
  SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN a.panjang ELSE 0 END) AS kering_yard
FROM
	db_finishing.tbl_produksi a
LEFT JOIN db_finishing.tbl_no_mesin b ON
	a.no_mesin = b.no_mesin
WHERE
  ...
  and a.proses IN('Oven Stenter (Normal)', 'Oven Kering (Normal)')");	
...
WHERE
  ...
  -- and a.nama_mesin IN('OVN1','OVN2','OVN3','OVN4','OVN5','OVN6')
  and a.proses IN('Oven Stenter (Normal)', 'Oven Tambah Obat (Khusus)', 'CURING STENTER (Bantu)')");		
$dts_B_stenter	= sqlsrv_fetch_array($sqlsB_stenter, SQLSRV_FETCH_ASSOC);

$sqlsB_stenter_ulang  = sqlsrv_query($conS, "SELECT
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN a.qty ELSE 0 END) AS basah,
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN 1 ELSE 0 END) AS basah_lot,
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN a.panjang ELSE 0 END) AS yard, 
  SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN a.qty ELSE 0 END) AS kering,
...
$dts_B_stenter_ulang	= sqlsrv_fetch_array($sqlsB_stenter_ulang, SQLSRV_FETCH_ASSOC);
...
$sqlB_stenter  = db2_exec($conn2, "SELECT SUM(USERPRIMARYQUANTITY) AS KG, COUNT(PRODUCTIONORDERCODE) AS LOT FROM ITXVIEWPRODUCTIONPROGRESSMULAI p
								...
								AND (p.OPERATIONCODE = 'OVN1'
									...
									-- OR p.OPERATIONCODE = 'OVN5'
									-- OR p.OPERATIONCODE = 'OVN6'
                  OR p.OPERATIONCODE = 'OVB1'
                  OR p.OPERATIONCODE = 'OVB2'
                  OR p.OPERATIONCODE = 'CUR2'
                  )
								-- AND p.MACHINECODE LIKE 'P3ST%'
								");		
$dt_B_stenter	= db2_fetch_assoc($sqlB_stenter);

$sqlB_stenter_ulang  = db2_exec($conn2, "SELECT SUM(USERPRIMARYQUANTITY) AS KG, COUNT(PRODUCTIONORDERCODE) AS LOT FROM ITXVIEWPRODUCTIONPROGRESSMULAI p
								WHERE 
								(p.KDKAIN = 'KFF' OR p.KDKAIN IS NULL)
							    AND TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) BETWEEN '$Awal 23:01:00' AND '$Akhir 23:00:00'
								AND ( p.OPERATIONCODE = 'OVN4'
									OR p.OPERATIONCODE = 'OVN5'
									OR p.OPERATIONCODE = 'OVN6'
                  )
								-- AND p.MACHINECODE LIKE 'P3ST%'
								");		
$dt_B_stenter_ulang	= db2_fetch_assoc($sqlB_stenter_ulang);
...
$sqlsB_fin_potong  = sqlsrv_query($conS, "SELECT
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN a.qty ELSE 0 END) AS basah,
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN 1 ELSE 0 END) AS basah_lot,
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN a.panjang ELSE 0 END) AS yard,
  SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN a.qty ELSE 0 END) AS kering,
...
WHERE
  ...
  CONCAT(a.tgl_update,CONCAT(' ',a.jam_in)) BETWEEN '$Awal 23:01' AND '$Akhir 23:00'
  and a.no_mesin LIKE 'P3ST%'
  and a.proses IN('Potong Pinggir (Normal)')");		
$dts_B_fin_potong	= sqlsrv_fetch_array($sqlsB_fin_potong, SQLSRV_FETCH_ASSOC);
...
WHERE
  ...
  and a.no_mesin LIKE 'P3ST%'
  and a.proses IN ('Finishing Ulang - Dyeing (Bantu)','Finishing Ulang - Dyeing 2 (Bantu)','Finishing Ulang - Dyeing 3 (Bantu)', 'Finishing Ulang - Naik Suhu Dyeing (Bantu)')");		
...
$sqlB_fin_bl  = db2_exec($conn2, "SELECT SUM(USERPRIMARYQUANTITY) AS KG, COUNT(PRODUCTIONORDERCODE) AS LOT FROM ITXVIEWPRODUCTIONPROGRESSMULAI p
								WHERE (p.KDKAIN = 'KFF' OR p.KDKAIN IS NULL)
								AND TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) BETWEEN '$Awal 23:01:00' AND '$Akhir 23:00:00'
								AND p.OPERATIONCODE IN ('OPW1','OPW2', 'OPW3', 'OPW4', 'BLD1', 'BLD2', 'BLD3', 'BLD4', 'BLP1', 'BLP2', 'BLP3', 'BLP4')");		
...
$sqlsB_lipat  = sqlsrv_query($conS, "SELECT
  SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN a.qty ELSE 0 END) AS kering,
	SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN 1 ELSE 0 END) AS kering_lot,
	SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN a.panjang ELSE 0 END) AS yard
FROM
	db_finishing.tbl_produksi a
LEFT JOIN db_finishing.tbl_no_mesin b ON
	a.no_mesin = b.no_mesin
WHERE
  ...
  and a.proses IN('Lipat (Normal)', 'tembak-tembak (Normal)', 'tembak-tembak (Normal)')");		
$dts_B_lipat	= sqlsrv_fetch_array($sqlsB_lipat, SQLSRV_FETCH_ASSOC);

$sqlsB_lipat_basah  = sqlsrv_query($conS, "SELECT
  SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN a.qty ELSE 0 END) AS basah,
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN 1 ELSE 0 END) AS basah_lot,
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN a.panjang ELSE 0 END) AS yard,
  SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN a.qty ELSE 0 END) AS kering,
...
WHERE
  ...
  --	and a.nama_mesin IN('LIP1')
  and a.proses IN('Lipat (Normal)', 'tembak-tembak (Normal)', 'tembak-tembak (Normal)')");		
$dts_B_lipat_basah	= sqlsrv_fetch_array($sqlsB_lipat_basah, SQLSRV_FETCH_ASSOC);	
...
WHERE
  ...
  CONCAT(a.tgl_update,CONCAT(' ',a.jam_in)) BETWEEN '$Awal 23:01' AND '$Akhir 23:00'
  --	and a.nama_mesin IN('LIP1')
  and a.proses IN('Oven Fleece (Normal)', 'Oven Fleece Ulang (Normal)')");		
...
$sqlsB_fin_1x  = sqlsrv_query($conS, "SELECT
  SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN a.qty ELSE 0 END) AS basah,
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN 1 ELSE 0 END) AS basah_lot,
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN a.panjang ELSE 0 END) AS yards,
  SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN a.qty ELSE 0 END) AS kering,
	SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN 1 ELSE 0 END) AS kering_lot,
	SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN a.panjang ELSE 0 END) AS kering_yard
FROM
...
$sqlB_preset  = db2_exec($conn2, "SELECT SUM(USERPRIMARYQUANTITY) AS KG, COUNT(PRODUCTIONORDERCODE) AS LOT FROM ITXVIEWPRODUCTIONPROGRESSMULAI p
								WHERE (p.KDKAIN = 'KFF' OR p.KDKAIN IS NULL)
								AND TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) BETWEEN '$Awal 23:01:00' AND '$Akhir 23:00:00'
								AND (p.OPERATIONCODE = 'PRE1' OR p.OPERATIONCODE = 'OVG1')");		
...
$sqlB_lipat = db2_exec($conn2, "SELECT SUM(USERPRIMARYQUANTITY) AS KG, COUNT(PRODUCTIONORDERCODE) AS LOT FROM ITXVIEWPRODUCTIONPROGRESSMULAI p
								WHERE (p.KDKAIN = 'KFF' OR p.KDKAIN IS NULL)
								AND TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) BETWEEN '$Awal 23:01:00' AND '$Akhir 23:00:00'
								AND p.OPERATIONCODE IN ('LIP1','LIP2', 'LIP3', 'LPS1', 'GKF1', 'INS5', 'INS6', 'TMF1', 'TRF1')");		
...
$sqlsK_tarik_lebar  = sqlsrv_query($conS, "SELECT
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN a.qty ELSE 0 END) AS basah,
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN 1 ELSE 0 END) AS basah_lot,
	SUM(CASE WHEN a.kondisi_kain2 = 'BASAH' THEN a.panjang ELSE 0 END) AS yard,
  SUM(CASE WHEN a.kondisi_kain2 = 'KERING' THEN a.qty ELSE 0 END) AS kering,
...
WHERE
  ...
  CONCAT(a.tgl_update,CONCAT(' ',a.jam_in)) BETWEEN '$Awal 23:01' AND '$Akhir 23:00' 
  -- and a.nama_mesin IN('FNJ2','FNJ3','FNJ4','FNJ5','FNJ6')
  and a.proses IN ('Tarik Lebar (Normal)', 'Tarik Lebar - Dyeing (Bantu)')");		
$dts_BK_tarik_lebar	= sqlsrv_fetch_array($sqlsK_tarik_lebar, SQLSRV_FETCH_ASSOC);
...
WHERE
  ...
  CONCAT(a.tgl_update,CONCAT(' ',a.jam_in)) BETWEEN '$Awal 23:01' AND '$Akhir 23:00'
  --	and a.nama_mesin IN('LIP1')
  and a.proses IN('Finishing Ulang - Dyeing (Bantu)','Finishing Ulang - Naik Suhu Dyeing (Bantu)')");		
...
$dt_K_lipat	= db2_fetch_assoc($sqlK_lipat);			
		?>	
          <tr>  
            ...
            <td align="left">OPW1,OPW2, OPW3 OPW4, BLD1, BLD2, BLD3, BLD4, BLP1, BLP2, BLP3, BLP4 </td>
            ...
            <td align="right"><?php echo number_format(round($dts_B_fin_bl['kering'],2),2); ?></td>
            <td align="center"><?php echo round($dts_B_fin_bl['kering_lot']); ?></td>
            <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['belah_c_basah']+(round($dt_B_fin_bl['KG'],2)-(round($dts_B_fin_bl['basah'],2)+round($dts_B_fin_bl['kering'],2))),2),2);?></td>
            <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['belah_c_basah_lot'])+($dt_B_fin_bl['LOT']-($dts_B_fin_bl['basah_lot']+$dts_B_fin_bl['kering_lot'])); ?></td>
          </tr>
          <tr>
            ...
            <td align="right"><?php echo number_format(round($dts_K_compact['basah'],2),2); ?></td>
            <td align="center"><?php echo round($dts_K_compact['basah_lot']); ?></td>
            ...
            <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['compact_n_kering']+(round($dt_K_compact['KG'],2)-(round($dts_K_compact['basah'],2)+round($dts_K_compact['kering'],2))),2),2);?></td>
            <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['compact_n_kering_lot'])+($dt_K_compact['LOT']-($dts_K_compact['basah_lot']+$dts_K_compact['kering_lot'])); ?></td>
          </tr>
			    <tr>
            ...
            <td align="right"><?php echo number_format(round($dts_K_compact_dye['basah'],2),2); ?></td>
            <td align="center"><?php echo round($dts_K_compact_dye['basah_lot']); ?></td>
            ...
            <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['compact_dye_kering']+(round($dt_K_compact_dye['KG'],2)-(round($dts_K_compact_dye['basah'],2)+round($dts_K_compact_dye['kering'],2))),2),2);?></td>
            <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['compact_dye_kering_lot'])+($dt_K_compact_dye['LOT']-($dts_K_compact_dye['basah_lot']+$dts_K_compact_dye['kering_lot'])); ?></td>
          </tr> 
			    <tr>
            <td align="left">Compact Perb. FIN</td>
            <td align="left">CPF1, CPF2, CPF3, CPF4</td>
            ...
            <td align="right"><?php echo number_format(round($dts_K_compact_fin['basah'],2),2); ?></td>
            <td align="center"><?php echo round($dts_K_compact_fin['basah_lot']); ?></td>
            ...
            <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['compact_fin_kering']+(round($dt_K_compact_fin['KG'],2)-(round($dts_K_compact_fin['basah'],2)+round($dts_K_compact_fin['kering'],2))),2),2);?></td>
            <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['compact_fin_kering_lot'])+($dt_K_compact_fin['LOT']-($dts_K_compact_fin['basah_lot']+$dts_K_compact_fin['kering_lot'])); ?></td>
          </tr>
			    <tr>
            ...
            <td align="right"><?php echo number_format(round($dts_B_fin_1x['kering'],2),2); ?></td>
            <td align="center"><?php echo round($dts_B_fin_1x['kering_lot']); ?></td>
            <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa1['belah_c_basah1']+(round($dt_B_fin_1x['KG'],2)-(round($dts_B_sisa['fin_1x_kering'],2)+round($dts_B_fin_1x['kering'],2))),2),2);?></td>
            <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['fin_1x_kering_lot'])+($dt_B_fin_1x['LOT']-($dts_B_fin_1x['basah_lot']+$dts_B_fin_1x['kering_lot'])); ?></td>
          </tr>
			    <tr>
            <td align="left">Finishing Jadi</td>
            <td align="left">FNJ1</td>
            <td align="right"><?php echo number_format(round($dts_B_sisa['fin_jadi_basah'] + $dts_B_sisa['fin_jadi_kering'] ,2),2);?></td>
            <td align="center"><?php echo number_format(round($dts_B_sisa['fin_jadi_basah_lot'] + $dts_B_sisa['fin_jadi_kering_lot'],0),0);?></td>
            ...
            <td align="right"><?php echo number_format(round($dts_B_fin_jadi['kering'],2),2); ?></td>
            <td align="center"><?php echo round($dts_B_fin_jadi['kering_lot']); ?></td>
            <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round(($dts_B_sisa['fin_jadi_basah']+ $dts_B_sisa['fin_jadi_kering'])+(round($dt_B_fin_jadi['KG'],2)-(round($dts_B_fin_jadi['basah'],2)+round($dts_B_fin_jadi['kering'],2))),2),2);?></td>
            <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['fin_jadi_basah_lot'])+ $dts_B_sisa['fin_jadi_kering_lot']+($dt_B_fin_jadi['LOT']-($dts_B_fin_jadi['basah_lot']+$dts_B_fin_jadi['kering_lot'])); ?></td>
          </tr>
			    <tr>
            ...
            <td align="right"><?php echo number_format(round($dts_B_fin_ulang['kering'],2),2); ?></td>
            <td align="center"><?php echo round($dts_B_fin_ulang['kering_lot']); ?></td>
            <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['fin_ul_basah']+(round($dt_B_fin_ulang['KG'],2)-(round($dts_B_fin_ulang['basah'],2)+round($dts_B_fin_ulang['kering'],2))),2),2);?></td>
            <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['fin_ul_basah_lot'])+($dt_B_fin_ulang['LOT']-($dts_B_fin_ulang['basah_lot']+$dts_B_fin_ulang['kering_lot'])); ?></td>
          </tr>
			    <tr>
            ...
            <td align="right"><?php echo number_format(round($dts_B_fin_ulang_BRS['kering'],2),2); ?></td>
            <td align="center"><?php echo round($dts_B_fin_ulang_BRS['kering_lot']); ?></td>
            <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['fin_ul_p_brs_basah']+(round($dt_B_fin_ulang_BRS['KG'],2)-(round($dts_B_fin_ulang_BRS['basah'],2)+round($dts_B_fin_ulang_BRS['kering'],2))),2),2);?></td>
            <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['fin_ul_p_brs_basah_lot'])+($dt_B_fin_ulang_BRS['LOT']-($dts_B_fin_ulang_BRS['basah_lot']+$dts_B_fin_ulang_BRS['kering_lot'])); ?></td>
          </tr>
        <tr>
          ...
          <td align="right"><?php echo number_format(round($dts_B_fin_ulang_DYE['kering'],2),2); ?></td>
          <td align="center"><?php echo round($dts_B_fin_ulang_DYE['kering_lot']); ?></td>
          <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['fin_ul_p_dye_basah']+(round($dt_B_fin_ulang_DYE['KG'],2)-(round($dts_B_fin_ulang_DYE['basah'],2)+round($dts_B_fin_ulang_DYE['kering'],2))),2),2);?></td>
          <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['fin_ul_p_dye_basah_lot'])+($dt_B_fin_ulang_DYE['LOT']-($dts_B_fin_ulang_DYE['basah_lot']+$dts_B_fin_ulang_DYE['kering_lot'])); ?></td>
		    </tr>
			<tr>
			  <td align="left">Lipat+Perbaikan</td>
			  <td align="left">LIP1, LIP2, LIP3, LPS1, GKF1, INS5, INS6, TMF1, TRF1</td>
			  <td align="right">0</td>
			  <td align="center">0</td>
			  <td align="right"><?php echo number_format(round($dt_B_lipat['KG'],2),2); ?></td>
			  <td align="center"><?php echo round($dt_B_lipat['LOT']); ?></td>
        <td align="right"><?php echo number_format(round($dts_B_lipat_basah['basah'],2),2); ?></td>
			  <td align="center"><?php echo round($dts_B_lipat_basah['basah_lot']); ?></td>
			  <td align="right"><?php echo number_format(round($dts_B_lipat_basah['kering'],2),2); ?></td>
			  <td align="center"><?php echo round($dts_B_lipat_basah['kering_lot']); ?></td>
			  <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa1['belah_c_basah']+(round($dt_B_lipat['KG'],2)-(round($dts_B_lipat_basah['basah'],2)+round($dts_B_lipat_basah['kering'],2))),2),2);?></td>
			  <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa1['belah_c_basah_lot'])+($dt_B_lipat['LOT']-($dts_B_lipat_basah['basah_lot']+$dts_B_lipat_basah['kering_lot'])); ?></td>
		  </tr>
			<tr>
			  <td align="left">Oven Kragh</td>
			  <td align="left">OVN1, OVN2</td>
        ...
        <td align="right"><?php echo number_format(round($dts_B_oven_k['kering'],2),2); ?></td>
        <td align="center"><?php echo round($dts_B_oven_k['kering_lot']); ?></td>
        <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['oven_k_basah']+(round($dt_B_oven_k['KG'],2)-(round($dts_B_oven_k['basah'],2)+round($dts_B_oven_k['kering'],2))),2),2);?></td>
        <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['oven_k_basah_lot'])+($dt_B_oven_k['LOT']-($dts_B_oven_k['basah_lot']+$dts_B_oven_k['kering_lot'])); ?></td>
		  </tr>
			<tr>
			  <td align="left">Oven Perb. Dye</td>
        ...
        <td align="center"><?php echo round($dts_B_pr_dye['basah_lot']); ?></td>
			  <td align="right"><?php echo number_format(round($dts_B_pr_dye['kering'],2),2); ?></td>
        <td align="center"><?php echo round($dts_B_pr_dye['kering_lot']); ?></td>
        <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['oven_p_dye_basah']+(round($dt_B_pr_dye['KG'],2)-(round($dts_B_pr_dye['basah'],2)+round($dts_B_pr_dye['kering'],2))),2),2);?></td>
        <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['oven_p_dye_basah_lot'])+($dt_B_pr_dye['LOT']-($dts_B_pr_dye['basah_lot']+$dts_B_pr_dye['kering_lot'])); ?></td>
		  </tr>
			<tr>
			  <td align="left">Oven Stenter</td>
			  <td align="left">OVN1, OVN3, OVB1, OVB2, CUR2</td>
        ...
        <td align="right"><?php echo number_format(round($dts_B_stenter['kering'],2),2); ?></td>
        <td align="center"><?php echo round($dts_B_stenter['kering_lot']); ?></td>
        <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['oven_b_st_basah']+(round($dt_B_stenter['KG'],2)-(round($dts_B_stenter['basah'],2)+round($dts_B_stenter['kering'],2))),2),2);?></td>
        <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['oven_b_st_basah_lot'])+($dt_B_stenter['LOT']-($dts_B_stenter['basah_lot']+$dts_B_stenter['kering_lot'])); ?></td>
		  </tr>
			<tr>
			  <td align="left">Oven Stenter Ulang</td>
			  <td align="left">OVN4, OVN5, OVN6</td>
			  <td align="right">0</td>
			  <td align="center">0</td>
        <td align="right"><?php echo number_format(round($dt_B_stenter_ulang['KG'],2),2);?></td>
        <td align="center"><?php echo number_format(round($dt_B_stenter_ulang['LOT'],2),0);?></td>
        <td align="right"><?php echo number_format(round($dts_B_stenter_ulang['basah'],2),2); ?></td>
        <td align="center"><?php echo round($dts_B_stenter_ulang['basah_lot']); ?></td>
        <td align="right"><?php echo number_format(round($dts_B_stenter_ulang['kering'],2),2); ?></td>
        <td align="center"><?php echo round($dts_B_stenter_ulang['kering_lot']); ?></td>
        <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['oven_k_basah']+(round($dt_B_stenter_ulang['KG'],2)-(round($dts_B_stenter_ulang['basah'],2)+round($dts_B_stenter_ulang['kering'],2))),2),2);?></td>
        <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['oven_k_basah_lot'])+($dt_B_stenter_ulang['LOT']-($dts_B_stenter_ulang['basah_lot']+$dts_B_stenter_ulang['kering_lot'])); ?></td>
		  </tr>
			<tr>
        ...
        <td align="right"><?php echo number_format(round($dts_K_padder['basah'],2),2); ?></td>
        <td align="center"><?php echo round($dts_K_padder['basah_lot']); ?></td>
        ...
        <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['padder_kering']+(round($dt_K_tambah_obat['KG'],2)-(round($dts_K_padder['basah'],2)+round($dts_K_padder['kering'],2))),2),2);?></td>
        <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['padder_kering_lot'])+($dt_K_padder['LOT']-($dts_K_padder['basah_lot']+$dts_K_padder['kering_lot'])); ?></td>
		  </tr>
			<tr>
        ...
        <td align="right"><?php echo number_format(round($dt_B_fin_jadi['KG'],2),2);?></td>
        <td align="center"><?php echo number_format(round($dt_B_fin_jadi['LOT'],2),0);?></td>
        <td align="right"><?php echo number_format(round($dts_B_fin_potong['basah'],2),2); ?></td>
        <td align="center"><?php echo round($dts_B_fin_potong['basah_lot']); ?></td>
        <td align="right"><?php echo number_format(round($dts_B_fin_potong['kering'],2),2); ?></td>
        <td align="center"><?php echo round($dts_B_fin_potong['kering_lot']); ?></td>
        <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['fin_jadi_basah']+(round($dt_B_fin_jadi['KG'],2)-(round($dts_B_fin_potong['basah'],2)+round($dts_B_fin_potong['kering'],2))),2),2);?></td>
        <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['fin_jadi_basah_lot'])+($dt_B_fin_jadi['LOT']-($dts_B_fin_potong['basah_lot']+$dts_B_fin_potong['kering_lot'])); ?></td>
		  </tr>
			<tr>
			  <td align="left">Preset</td>
			  <td align="left">PRE1, OVG1</td>
        ...
        <td align="right"><?php echo number_format(round($dts_B_preset['kering'],2),2); ?></td>
			  <td align="center"><?php echo round($dts_B_preset['kering_lot']); ?></td>
			  <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['preset_kering']+(round($dt_B_preset['KG'],2)-(round($dts_B_preset['basah'],2)+round($dts_B_preset['kering'],2))),2),2);?></td>
			  <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['preset_kering_lot'])+($dt_B_preset['LOT']-($dts_B_preset['basah_lot']+$dts_B_preset['kering_lot'])); ?></td>
		  </tr>
      <tr>
        ...
        <td align="right"><?php echo number_format(round($dts_K_steamer['basah'],2),2); ?></td>
        <td align="center"><?php echo round($dts_K_steamer['basah_lot']); ?></td>
        ...
        <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['steamer_kering']+(round($dt_K_steamer['KG'],2)-(round($dts_K_steamer['basah'],2)+round($dts_K_steamer['kering'],2))),2),2);?></td>
        <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['steamer_kering_lot'])+($dt_K_steamer['LOT']-($dts_K_steamer['basah_lot']+$dts_K_steamer['kering_lot'])); ?></td>
		  </tr>
      <tr>
			  <td align="left">Tarik Lebar</td>
			  <td align="left">FNJ1, FNJ2, FNJ3, FNJ4, FNJ5, FNJ6</td>
        ...
        <td align="right"><?php echo number_format(round($dts_BK_tarik_lebar['basah'],2),2); ?></td>
        <td align="center"><?php echo round($dts_BK_tarik_lebar['basah_lot']); ?></td>
        <td align="right"><?php echo number_format(round($dts_BK_tarik_lebar['kering'],2),2); ?></td>
        <td align="center"><?php echo round($dts_BK_tarik_lebar['kering_lot']); ?></td>
        <td align="right" bgcolor="#B9E1F0"><?php echo number_format(round($dts_B_sisa['fin_ul_kering']+(round($dt_K_fin_ulang['KG'],2)-(round($dts_BK_tarik_lebar['basah'],2)+round($dts_BK_tarik_lebar['kering'],2))),2),2);?></td>
        <td align="center" bgcolor="#B9E1F0"><?php echo ($dts_B_sisa['fin_ul_kering_lot'])+($dt_K_fin_ulang['LOT']-($dts_BK_tarik_lebar['basah_lot']+$dts_BK_tarik_lebar['kering_lot'])); ?></td>
    </tr>
        </tbody>
		<tfoot> 
		<?php
        ...
        $dt_B_fin_bl['LOT'] +
        $dt_K_compact['LOT'] +
        $dt_K_compact_dye['LOT'] +
        $dt_K_compact_fin['LOT'] +
        $dt_B_fin_1x['LOT'] +
        $dt_B_fin_jadi['LOT'] +
        $dt_B_fin_ulang['LOT'] +
        $dt_B_fin_ulang_BRS['LOT'] +
        $dt_B_fin_ulang_DYE['LOT'] +
        $dt_B_lipat['LOT'] +
        $dt_B_oven_k['LOT'] +
        $dt_B_pr_dye['LOT'] +
        $dt_B_stenter['LOT'] +
        $dt_B_stenter_ulang['LOT'] +
        $dt_K_padder['LOT'] +
        $dt_B_fin_jadi['LOT'] +
        $dt_B_preset['LOT'] +
        $dt_K_steamer['LOT'] +
        round($dt_K_fin_ulang['LOT'],2)
        ;
//sekarang lagi cek disini
			$total_b_masuk_kg 	= 
        round($dt_B_fin_bl['KG'],2) +
        round($dt_K_compact['KG'],2) +
        round($dt_K_compact_dye['KG'],2) +
        round($dt_K_compact_fin['KG'],2) +
        round($dt_B_fin_1x['KG'],2) +
        round($dt_B_fin_jadi['KG'],2) +
        round($dt_B_fin_ulang['KG'],2) +
        round($dt_B_fin_ulang_BRS['KG'],2) +
        round($dt_B_fin_ulang_DYE['KG'],2) +
        round($dt_B_lipat['KG'],2) +
        round($dt_B_oven_k['KG'],2) +
        round($dt_B_pr_dye['KG'],2) +
        round($dt_B_stenter['KG'],2) +
        round($dt_B_stenter_ulang['KG'],2) +
        round($dt_K_padder['KG'],2) +
        round($dt_B_fin_jadi['KG'],2) +
        round($dt_B_preset['KG'],2) +
        round($dt_K_steamer['KG'],2) +
        round($dt_K_fin_ulang['KG'],2)
        ;

			$total_b_keluar_lot 	= 
        $dts_B_fin_bl['basah_lot'] +
        $dts_K_compact['basah_lot'] +
        $dts_K_compact_dye['basah_lot'] +
        $dts_K_compact_fin['basah_lot'] +
        $dts_B_fin_1x['basah_lot'] +
        $dts_B_fin_jadi['basah_lot'] +
        $dts_B_fin_ulang['basah_lot'] +
        $dts_B_fin_ulang_BRS['basah_lot'] +
        $dts_B_fin_ulang_DYE['basah_lot'] +
        $dts_B_lipat_basah['basah_lot'] +
        $dts_B_oven_k['basah_lot'] +
        $dts_B_pr_dye['basah_lot'] +
        $dts_B_stenter['basah_lot'] +
        $dts_B_stenter_ulang['basah_lot'] +
        $dts_K_padder['basah_lot'] +
        $dts_B_fin_potong['basah_lot'] +
        $dts_B_preset['basah_lot'] +
        $dts_K_steamer['basah_lot'] +
        $dts_BK_tarik_lebar['basah_lot']
        ;

			$total_b_keluar_kg 	= 
        $dts_B_fin_bl['basah'] +
        $dts_K_compact['basah'] +
        $dts_K_compact_dye['basah'] +
        $dts_K_compact_fin['basah'] +
        $dts_B_fin_1x['basah'] +
        $dts_B_fin_jadi['basah'] +
        $dts_B_fin_ulang['basah'] +
        $dts_B_fin_ulang_BRS['basah'] +
        $dts_B_fin_ulang_DYE['basah'] +
        $dts_B_lipat_basah['basah'] +
        $dts_B_oven_k['basah'] +
        $dts_B_pr_dye['basah'] +
        $dts_B_stenter['basah'] +
        $dts_B_stenter_ulang['basah'] +
        $dts_K_padder['basah'] +
        $dts_B_fin_potong['basah'] +
        $dts_B_preset['basah'] +
        $dts_K_steamer['basah'] +
        $dts_BK_tarik_lebar['basah']
        ;

      $total_k_keluar_lot 	= 
        round($dts_B_fin_bl['kering_lot']) +
        round($dts_K_compact['kering_lot']) +
        round($dts_K_compact_dye['kering_lot']) +
        round($dts_K_compact_fin['kering_lot']) +
        round($dts_B_fin_1x['kering_lot']) +
        round($dts_B_fin_jadi['kering_lot']) +
        round($dts_B_fin_ulang['kering_lot']) +
        round($dts_B_fin_ulang_BRS['kering_lot']) +
        round($dts_B_fin_ulang_DYE['kering_lot']) +
        round($dts_B_lipat_basah['kering_lot']) +
        round($dts_B_oven_k['kering_lot']) +
        round($dts_B_pr_dye['kering_lot']) +
        round($dts_B_stenter['kering_lot']) +
        round($dts_B_stenter_ulang['kering_lot']) +
        round($dts_K_padder['kering_lot']) +
        round($dts_B_fin_potong['kering_lot']) +
        round($dts_B_preset['kering_lot']) +
        round($dts_K_steamer['kering_lot']) +
        round($dts_BK_tarik_lebar['kering_lot'],2)
        ;

			$total_k_keluar_kg 	= 
        $dts_B_fin_bl['kering'] +
        $dts_K_compact['kering'] +
        $dts_K_compact_dye['kering'] +
        $dts_K_compact_fin['kering'] +
        $dts_B_fin_1x['kering'] +
        $dts_B_fin_jadi['kering'] +
        $dts_B_fin_ulang['kering'] +
        $dts_B_fin_ulang_BRS['kering'] +
        $dts_B_fin_ulang_DYE['kering'] +
        $dts_B_lipat_basah['kering'] +
        $dts_B_oven_k['kering'] +
        $dts_B_pr_dye['kering'] +
        $dts_B_stenter['kering'] +
        $dts_B_stenter_ulang['kering'] +
        $dts_K_padder['kering'] +
        $dts_B_fin_potong['kering'] +
        $dts_B_preset['kering'] +
        $dts_K_steamer['kering'] +
        $dts_BK_tarik_lebar['kering'];

			$total_b_sisa = 
        $dts_B_sisa['belah_c_basah'] +
        $dts_B_sisa['compact_n_kering'] +
        $dts_B_sisa['compact_dye_kering'] +
        $dts_B_sisa['compact_fin_kering'] +
        $dts_B_sisa['fin_1x_kering'] +
        $dts_B_sisa['fin_jadi_basah'] + $dts_B_sisa['fin_jadi_kering'] +
        $dts_B_sisa['fin_ul_basah'] + 
        $dts_B_sisa['fin_ul_p_brs_basah'] +
        $dts_B_sisa['fin_ul_p_dye_basah'] +
        $dts_B_sisa['oven_k_basah'] +
        $dts_B_sisa['oven_p_dye_basah'] +
        $dts_B_sisa['oven_b_st_basah'] +
        $dts_B_sisa['padder_kering'] +
        $dts_B_sisa['pot_pinggir_kering'] +
        $dts_B_sisa['preset_kering'] +
        $dts_B_sisa['steamer_kering'] +  
        $dts_B_sisa['fin_ul_kering'] 
        ;

			$total_b_sisa_lot = 
        $dts_B_sisa['belah_c_basah_lot'] +
        $dts_B_sisa['compact_n_kering_lot'] +
        $dts_B_sisa['compact_dye_kering_lot'] +
        $dts_B_sisa['compact_fin_kering_lot'] +
        $dts_B_sisa['fin_1x_kering_lot'] +
        $dts_B_sisa['fin_jadi_basah_lot'] + $dts_B_sisa['fin_jadi_kering_lot'] +
        $dts_B_sisa['fin_ul_basah_lot'] + 
        $dts_B_sisa['fin_ul_p_brs_basah_lot'] +
        $dts_B_sisa['fin_ul_p_dye_basah_lot'] +
        $dts_B_sisa['oven_k_basah_lot'] +
        $dts_B_sisa['oven_p_dye_basah_lot'] +
        $dts_B_sisa['oven_b_st_basah_lot'] +
        $dts_B_sisa['padder_kering_lot'] +
        $dts_B_sisa['pot_pinggir_kering_lot']  +
        $dts_B_sisa['preset_kering_lot']  +
        $dts_B_sisa['steamer_kering_lot'] +
        $dts_B_sisa['fin_ul_kering_lot']
        ;
      
    ?>
    <tr>
      <td align="center" bgcolor="#FBF330">TOTAL</td>
      ...
      <td align="center" bgcolor="#FBF330"><?php echo number_format($total_k_keluar_lot,0); ?></td>
      <td align="right" bgcolor="#FBF330"><?php echo number_format(($total_b_sisa) + ($total_b_masuk_kg - ($total_b_keluar_kg + $total_k_keluar_kg)),2); ?></td>
      <td align="center" bgcolor="#FBF330"><?php echo number_format(($total_b_sisa_lot) + ($total_b_masuk_lot - ($total_b_keluar_lot + $total_k_keluar_lot)),0); ?></td>
  </tr>
