    ...
    <tr>
	  ...
	  <?php
                ...
                for ($bulan = 1; $bulan <= 12; $bulan++) {
					$nama_bulan = DateTime::createFromFormat('!m', $bulan)->format('F'); // contoh: January
					$ada_data = isset($tanggal_ada_data[$bulan]);
                    // Default qty kosong
                    $qty = '-';

                    echo "<tr>";
                    echo "<td style='border: 1px solid black;' align='center'>{$nama_bulan}</td>";
                    // Jika tanggal di loop masih <= tanggal input, jalankan query
						...
						$start_time = $tanggal_awal_bulan->modify('-1 day')->format('Y-m-d 23:01:00');

						// Reset lagi ke tanggal pertama untuk hitung akhir bulan
						$tanggal_awal_bulan = DateTime::createFromFormat('Y-n-j', "$tahun-$bulan-01");
						$end_time = $tanggal_awal_bulan->modify('last day of this month')->format('Y-m-d 23:00:00');
                        $query_table1="SELECT
                                            ...
                                            (
											  SELECT SUM(jml_kk) FROM (
											    SELECT 
											      COUNT(DISTINCT tp2.nokk) AS jml_kk
											    FROM tbl_produksi tp2
											    WHERE tp2.tgl_buat BETWEEN '$start_time' AND '$end_time'
											    GROUP BY 
											      DATE(
											        CASE 
											          WHEN TIME(tp2.tgl_buat) <= '23:00:00' THEN DATE(tp2.tgl_buat)
											          ELSE DATE_ADD(DATE(tp2.tgl_buat), INTERVAL 1 DAY)
											        END
											      )
											  ) AS sub_total_kk
											) AS total_kk
                        ...
                        // Hari kerja
                            ...
                            $hari_kerja = mysqli_num_rows($hari_kerja_result) > 0 ? ($row_hari['jmlhari'])-1 : '0';
