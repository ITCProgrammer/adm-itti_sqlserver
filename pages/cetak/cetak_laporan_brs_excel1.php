      ...
      <td width="30%" align="left" valign="top">
                            ...
                            while($row_grk_bantu = mysqli_fetch_assoc($result_grk_bantu)){
								...
								$queryAPf1 = "WITH StepRank AS (  
									...
									WHERE 
										...
										AND ds.PRODUCTIONDEMANDCODE = '$row_grk_bantu[nodemand]' 
								),
								AllSteps AS (
									SELECT 
		...
		<table border="1" class="table-list1" width="100%">
                        ...
                        <tr>
							...
							// Query for Garuk Fleece FIN Final
                            $query_grk_fleece_final = "
                                SELECT
                                    SUM(qty) AS qty_grk_fleece,
                                    COUNT(*) AS jumlah_kk
                                FROM (
                                    SELECT 
                                        nokk,
                            ...
                            ";
                            $result_grk_fleece_final = mysqli_query($conb, $query_grk_fleece_final);
                            $row_grk_fleece_final = mysqli_fetch_assoc($result_grk_fleece_final);	
                            ?>
...
-->
							<td style="text-align:center;"><?= number_format(($row_grk_fleece_final['jumlah_kk']+$TjkGarukBantuFL ) ?? 0); ?></td>
                            <td style="text-align:center;"><?= number_format(($row_grk_fleece_final['qty_grk_fleece']+$TqtyGarukBantuFL ) ?? 0, 2); ?></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            // Query for Garuk AP FIN Final
                            $query_grk_ap = "
                                SELECT
                                    SUM(qty) AS qty_grk_ap,
                                    COUNT(*) AS jumlah_kk
                                FROM (
                                    SELECT 
                                        nokk,
                            ...
                            ";
                            $result_grk_ap = mysqli_query($conb, $query_grk_ap);
                            $row_grk_ap = mysqli_fetch_assoc($result_grk_ap);							
...
-->
							<td style="text-align:center;"><?= number_format(($row_grk_ap['jumlah_kk']+$TjkGarukBantuAP) ?? 0) ?></td>
                            <td style="text-align:center;"><?= number_format(($row_grk_ap['qty_grk_ap']+$TqtyGarukBantuAP) ?? 0, 2) ?></td>
                        </tr>
                        <tr>
                            <?php
                            ...
                            $query_potongbulu_lainlain = "
                                ...
                                FROM (
                                    SELECT 
                                        nokk,
                                        GROUP_CONCAT( id ORDER BY id SEPARATOR ', ') AS nodemand,
                                        MAX(langganan) AS langganan,
                                        MAX(proses) AS proses,
                                        SUM(qty) AS qty
                                    FROM
...
//                                ((int)($rowAPf['JUMLAHKK'] ?? 0)) +
								((int)($row_grk_fleece_final['jumlah_kk'] ?? 0)) +
								((int)($row_grk_ap['jumlah_kk'] ?? 0)) +
...
//                                ((float)($rowAPf['TOTAL_QTY'] ?? 0))+
								...
								((float)($row_grk_ap['qty_grk_ap'] ?? 0)) +
								((float)($row_grk_fleece_final['qty_grk_fleece'] ?? 0)) +
...
//                       $total_qty_garukfleece = (($row_garuk_fleece['TOTAL_QTY']) ?? 0) - (($rowFLEECEF3C20069['TOTAL_QTY']+$rowFLEECEF3C20069F['TOTAL_QTY']+$rowFLEECE['TOTAL_QTY']+$rowFINAL['TOTAL_QTY']) ?? 0);
					   $total_kk_garukfleece = (($row_garuk_fleece['JUMLAHKK']) ?? 0) - (($row_grk_fleece_final['jumlah_kk']+$row_grk_ap['jumlah_kk']) ?? 0);
                       $total_qty_garukfleece = (($row_garuk_fleece['TOTAL_QTY']) ?? 0) - (($row_grk_fleece_final['qty_grk_fleece']+$row_grk_ap['qty_grk_ap']) ?? 0);
                    ?>
...
//                ";
              ...
              <?php
                ...
                $query = "select
                            ...
                            where
                                rn = 1
                                and (durasi_jam <= 20 or durasi_jam is null)) as t
                            ";
                $result = mysqli_query($conb, $query);
                    			...
                    			FROM (
								 SELECT
									...
									WHERE										
										TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) >= TIMESTAMP('$tglAwal_tbl4')
                                        AND TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) <= TIMESTAMP('$tglAkhir_tbl4')
										AND pd.DLVSALORDLINESALORDCNTCODE IN ('REPLCEXP')
										AND p.OPERATIONCODE IN (
		                                    'AIR1', 'COM1', 'COM2', 'POL1', 'RSE1', 'RSE2', 'RSE3', 'RSE4', 'RSE5',
		                                    'SHR1', 'SHR2', 'SHR3', 'SHR4', 'SHR5', 'SUE1', 'SUE2', 'SUE3', 'SUE4',
		                                    'TDR1', 'WET1', 'WET2', 'WET3', 'WET4'
