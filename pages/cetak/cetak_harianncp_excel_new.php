      ...
      <td>
        <table width="100%" border="1" class="table-list1">
          <tbody>
            ...
            $qry1 = sqlsrv_query($cond, "SELECT * $FR2A FROM db_qc.tbl_ncp_qcf_now WHERE $Wdept $Wkategori CAST(tgl_buat as date) BETWEEN '$Awal' AND '$Akhir' $sts $WR2A 
	$GR2A ");
...
//$rowSVR=sqlsrv_fetch_array($qrySVR);
             ...
             <tr valign="top">
                ...
                <td align="center"><?php
                $dt_buat = $row1['tgl_buat'];
                if ($dt_buat instanceof DateTime) {
                  echo $dt_buat->format('d/m/y');
                } else {
                  echo date("d/m/y", strtotime($dt_buat));
                }
                ?></td>
                ...
                <td align="center">
                  <?php
                  if ($row1['tgl_rencana'] != "") {
                    $dt_rencana = $row1['tgl_rencana'];
                    if ($dt_rencana instanceof DateTime) {
                      echo $dt_rencana->format('d/m/y');
                    } else {
                      echo date("d/m/y", strtotime($dt_rencana));
                    }
                  }
                  ?></td>
                <td align="center">
                  <?php
                  if ($row1['tgl_selesai'] != "") {
                    $dt_selesai = $row1['tgl_selesai'];
                    if ($dt_selesai instanceof DateTime) {
                      echo $dt_selesai->format('d/m/y');
                    } else {
                      echo date("d/m/y", strtotime($dt_selesai));
                    }
                  }
                  ?></td>
                <td align="left"><?php echo strtoupper($row1['ket_analisa']); ?></td>
                <td align="left"><?php echo strtoupper($row1['akar_masalah']); ?></td>
                <td align="left"><?php echo strtoupper($row1['solusi_panjang']); ?></td>
                <td align="left"><a
                    href="http://10.0.0.10/online/logscan.php?kk=<?php echo $rowSVR['ID']; ?>">'<?php echo $row1['nokk']; ?></a>
                </td>
                <td align="left"><a target="_BLANK"
                    href="http://10.0.0.10/laporan/ppc_filter_steps.php?prod_order=<?= $row1['prod_order']; ?>"><?= $row1['prod_order']; ?></a>
                </td> <!-- NO KARTU KERJA -->
                <td align="left"><a target="_BLANK"
                    href="http://10.0.0.10/laporan/ppc_filter_steps.php?demand=<?= $row1['nodemand']; ?>"><?= $row1['nodemand']; ?></a>
                </td> <!-- NO DEMAND -->
              </tr>
              <?php $no++;
