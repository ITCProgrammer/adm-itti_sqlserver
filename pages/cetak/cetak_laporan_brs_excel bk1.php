...
<?php
        ...
        $qry_ncp = "SELECT
						...
						SUM(CASE WHEN masalah IN ('Bulu Garukan Reject', 'Bulu Garukan Tipis', 'Bulu Garukan Tebal', 'Bulu Garukan Tidak Rata',
                        'Bulu Garukan Rontok', 'Botak', 'Bulu Garukan Berbiji', 'Bulu Kain Panjang', 'Tidak Kena Garuk', 'Flamability Reject', 'After Wash Reject',
                        'Bulkines Reject', 'Permukaan Kain Berbulu', 'Bulu Garukan Pecah', 'Permukaan Reject', 'Serat Bengkok', 'Serat Renggang', 'Strectch Mati', 
                        'Stretch Kurang Elastis', 'Garukan di Muka Kain', 'Horizon', 'Thickness Reject',' Mengkerut') THEN berat ELSE 0 END) AS garuk_fleece,
                        SUM(CASE WHEN masalah IN ('Kaitan Jarum','Snagging','Bekas Tarik-tarik Snagging','Serat Pecah') THEN berat ELSE 0 END) AS garuk_ap,
                        SUM(CASE WHEN masalah IN ('Biji Anti Pilling Reject','Biji Anti Pilling Tidak Rata','Biji Anti Pilling Besar','Biji Anti Pilling Kecil','Pilling Reject','Appearance Jelek','Gesekan') THEN berat ELSE 0 END) AS oven_ap,
                        SUM(CASE WHEN masalah IN ('Garis Vertikal','Garis Diagonal','Horizontal','Shading (Garukan, Peachskin, Potong Bulu)','Rapuh','Kena Peach Skin di Terry','Beda Warna') THEN berat ELSE 0 END) AS peach_skin,
                        SUM(CASE WHEN masalah IN ('Kena Potong Shearing','Terry Putus','Sobek','Lebar Reject','Lebar Kurang','Lebar Lebih','Kena Pisau Garuk') THEN berat ELSE 0 END) AS pb_lain,
                        SUM(CASE WHEN masalah IN ('Kotor Tanah','Kontaminasi','Pinggiran Kain Kuning','Kena Warna','Kena Minyak','Kotor Kapas','Bintik Oksidasi','Gosong',
            			'Belang Steam','Belang Yang Hwa','Belang Spiral','Bercak Steam','Bolong','Bocor','Cabut-Cabut','Peach Skin Reject','Peachskin Tipis','Peachskin Tebal','Peachskin Tidak Rata',
            			'Serat Kayu','Tidak Kena Peach Skin','Beda Roll','Bursting Strength Reject','Salah Peach Skin','Peached Berbiji','Gramasi Ringan','Gramasi Berat','Cakar Ayam',
            			'Krismark','Garis Lipat','Keriput','Bekas Lipatan Anti Pilling','Gesekan Anti Pilling','Kusut','Kena Angin','Handfeel Reject','Handfeel Kurang Soft','Luntur') THEN berat ELSE 0 END) AS oven_ap_lain 
                    FROM
                        tbl_ncp_qcf_now
                    WHERE
                            ...
                            // Query DB2 for summary of USERPRIMARYQUANTITY and jumlah KK for GARUK KAIN FLEECE (RSE1-RSE5) yang TIDAK ADA step TDR1
                             $query_garuk_fleece = "WITH base_data AS (
									SELECT
										...
										pd.CODE AS DEMANDNO,
										p.OPERATIONCODE,
										p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME,
									...
									WHERE
										...
										AND p.OPERATIONCODE IN ('RSE1', 'RSE2', 'RSE3', 'RSE4', 'RSE5','WET1', 'WET2', 'WET3', 'WET4')
									...
									GROUP BY p.PRODUCTIONORDERCODE, p.OPERATIONCODE,
										p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME,pd.CODE
								),
								...
								jumlah_kk AS (
									-- SELECT COUNT(DISTINCT DEMANDNO) AS JUMLAHKK FROM base_data
										SELECT COUNT(*) AS JUMLAHKK
										FROM (
										  SELECT DISTINCT DEMANDNO, OPERATIONCODE
										  FROM base_data
										) AS unik
								)
      ...
      <td width="30%" align="left" valign="top">
		...
		// Query for Garuk Bantu
                            ...
                            $query_grk_bantu = "
                                SELECT 
                                        nokk,
                                        GROUP_CONCAT(nodemand ORDER BY id SEPARATOR ', ') AS nodemand,
                                    ...
                                    WHERE
										...
										'GARUK BANTU - DYG (Bantu)',
										'GARUK BANTU - FIN (Bantu)',
										'GARUK SLIGHTLY BRUS (Normal)',
										'WET SUEDING GREIGE FACE (Normal)'
										)                                        
                                    GROUP BY id";
                            $result_grk_bantu = mysqli_query($conb, $query_grk_bantu);
                            while($row_grk_bantu = mysqli_fetch_assoc($result_grk_bantu)){
								$demand1=trim($row_grk_bantu['nodemand']);
								$queryAPf1 = "WITH StepRank AS (  
									SELECT 
										p.PRODUCTIONORDERCODE,
										ds.OPERATIONCODE,
										ds.STEPNUMBER,
									...
									WHERE 
										...
										AND ds.PRODUCTIONDEMANDCODE = '$demand1' 
								),
								AllSteps AS (
									SELECT 
								...
								FROM Filtered;        
                                    ";
                            $resultAPf1 = db2_exec($conn2, $queryAPf1);
                            $rowAPf1 = db2_fetch_assoc($resultAPf1);	
							
								if($rowAPf1['JUMLAHKK']>0){
									$jkGarukBantuAP = "1";
									$qtyGarukBantuAP = $row_grk_bantu['qty'];
									
								}else if($row_grk_bantu['nodemand']!=""){
									$jkGarukBantuFL = "1";
									$qtyGarukBantuFL = $row_grk_bantu['qty'];
								}
								$TjkGarukBantuAP+=$jkGarukBantuAP;
								$TqtyGarukBantuAP+=$qtyGarukBantuAP;
								$TjkGarukBantuFL+=$jkGarukBantuFL;
								$TqtyGarukBantuFL+=$qtyGarukBantuFL;
							}
		?>
		<table border="1" class="table-list1" width="100%">
                        <tr>
                        ...
                        <tr>
                            ...
                            $F3C20069 = "WITH StepRank AS (  
									...
									Filtered AS (
										...
										JOIN AllSteps s 
											ON r.PRODUCTIONDEMANDCODE = s.PRODUCTIONDEMANDCODE
											AND s.STEPNUMBER > r.STEPNUMBER2
											AND s.OPERATIONCODE = 'FIN1'
										WHERE NOT EXISTS (
											SELECT 1 FROM AllSteps x
											WHERE 
												x.PRODUCTIONDEMANDCODE = r.PRODUCTIONDEMANDCODE
												AND x.STEPNUMBER > r.STEPNUMBER2
												AND x.STEPNUMBER < s.STEPNUMBER
												AND x.OPERATIONCODE NOT LIKE 'WAIT%'
												AND x.OPERATIONCODE NOT LIKE 'NCP%'
										)
                                    ...
                                    Total_qty AS(
                                        SELECT 
											...
											STEPNUMBER2 AS RSE_STEP,
											MIN(STEPNUMBER) AS FIN1_STEP
                                            FROM Filtered
                                            GROUP BY PRODUCTIONORDERCODE, TOTALPRIMARYQUANTITY, STEPNUMBER2
                                    ),
									jumlah_kk AS (
										-- SELECT COUNT(DISTINCT DEMANDNO) AS JUMLAHKK FROM base_data
										SELECT COUNT(*) AS JUMLAHKK
										FROM (
										  SELECT DISTINCT PRODUCTIONDEMANDCODE, OPERATIONCODE
										  FROM Filtered
										) AS unik
									)

									SELECT 
										(SELECT JUMLAHKK FROM jumlah_kk ) JUMLAHKK,
										(SELECT SUM(TOTALPRIMARYQUANTITY) AS TOTAL_QTY FROM Total_qty) TOTAL_QTY
                        ...
                        <tr>
                            ...
                            $query_all_data="
                                ...
                                WHERE
                                    tgl_buat BETWEEN '$tglAwal_tbl4' AND '$tglAkhir_tbl4' ";
                            $result_all_data=mysqli_query($conb, $query_all_data);

                            $grk_fleece_final_array=['GARUK FLEECE (Normal)',
										'GARUK FLEECE ULANG-BRS (Ulang)',
										'GARUK FLEECE ULANG-DYE (Ulang)',
										'GARUK FLEECE ULANG-FIN (Ulang)'];

                            $grk_ap_array=['GARUK ANTI PILLING (Normal)',
										'GARUK ANTI PILLING-BRS (Ulang)',
										'GARUK ANTI PILLING-CQA (Ulang)',
										'GARUK ANTI PILLING-DYE (Ulang)',
										'GARUK ANTI PILLING-FIN (Ulang)'];

                            $potongbulu_lainlain_array=['Potong Bulu (Bantu)',
										'POTONG BULU 07 (Bantu)',
										'POTONG BULU ANTI PILLING (Normal)',
										'POTONG BULU BACK BANTU - QC (Ulang)',
										...
										'POTONG BULU GREIGE (Normal)',
     									'POTONG BULU FLEECE (Normal)',										
										'POTONG BULU PEACH SKIN (Normal)'];

                            $anti_pilling_lainlain_array=['ANTI PILLING BANTU - DYE (Bantu)',
										'ANTI PILLING BANTU - FIN (Bantu)',
										'ANTI PILLING BANTU - QC (Bantu)',
										'ANTI PILLING BANTU - TAS (Bantu)',
										'ANTI PILLING BANTU-DYEING (Bantu)',
										...
										'ANTI PILLING (Khusus)',
										'ANTI PILLING (Normal)',
										'ANTI PILLING BIASA (Normal)',
										'ANTI PILLING (BIASA) (Normal)',
										'ANTI PILLING NORMAL (Normal)',
										...
										'ANTI PILLING LAIN-LAIN KHUSUS-FIN (Ulang)',
										'ANTI PILLING LAIN-LAIN-CQA (Ulang)'];

                            $ario_array=['AIRO (Normal)'];
                            
                            $sisir_array=['SISIR ANTI PILLING (Normal)',
										'SISIR BANTU (FIN) (Bantu)',
										'SISIR LAIN-LAIN (Bantu)'];

                            $peach_array=['PEACH SKIN (Normal)',
									'PEACH SKIN (Bantu)',
									'PEACH SKIN ULANG (Normal)',
									'PEACHSKIN ULANG-CQA (Ulang)',
									'PEACHSKIN ULANG-DYE (Ulang)',
									'PEACHSKIN ULANG-FIN (Ulang)',
									'PEACHSKIN ULANG-BRS (Ulang)',
                            ...
                            $peach_greige_array=['PEACHSKIN GREIGE (Normal)'];

                            $garuk_greige_array=['GARUK GREIGE (Normal)'];

                            $garuk_fleece_tambah_obat_array=['GARUK FLEECE Tambah Obat (Normal)'];

                            $data_all=array();
                            $array_nodemand=array();
                            while($ra=mysqli_fetch_assoc($result_all_data)){
                                $noDemand=trim($ra['nodemand']," ");
                                $proses=$ra['proses'];
                                $qty=$ra['qty'];
                                if(in_array($proses,$grk_fleece_final_array)){
                                    if(!in_array($noDemand,$array_nodemand)){
                                        $data_all['grk_fleece_final']['JUMLAHKK']=floatval($data_all['grk_fleece_final']['JUMLAHKK'])+1;
                                        $array_nodemand[]=$noDemand;
                                    }
                                    $data_all['grk_fleece_final']['TOTALQTY']=floatval($data_all['grk_fleece_final']['TOTALQTY'])+$qty;
                                }

                                else if(in_array($proses,$grk_ap_array)){
                                    if(!in_array($noDemand,$array_nodemand)){
                                        $data_all['grk_ap']['JUMLAHKK']=floatval($data_all['grk_ap']['JUMLAHKK'])+1;
                                        $array_nodemand[]=$noDemand;
                                    }
                                    $data_all['grk_ap']['TOTALQTY']=floatval($data_all['grk_ap']['TOTALQTY'])+$qty;
                                }
 
                                else if(in_array($proses,$potongbulu_lainlain_array)){
                                    if(!in_array($noDemand,$array_nodemand)){
                                        $data_all['potongbulu_lainlain']['JUMLAHKK']=floatval($data_all['potongbulu_lainlain']['JUMLAHKK'])+1;
                                        $array_nodemand[]=$noDemand;
                                    }
                                    $data_all['potongbulu_lainlain']['TOTALQTY']=floatval($data_all['potongbulu_lainlain']['TOTALQTY'])+$qty;
                                }

                                else if(in_array($proses,$anti_pilling_lainlain_array)){
                                    if(!in_array($noDemand,$array_nodemand)){
                                        $data_all['anti_pilling_lainlain']['JUMLAHKK']=floatval($data_all['anti_pilling_lainlain']['JUMLAHKK'])+1;
                                        $array_nodemand[]=$noDemand;
                                    }
                                    $data_all['anti_pilling_lainlain']['TOTALQTY']=floatval($data_all['anti_pilling_lainlain']['TOTALQTY'])+$qty;
                                }
   
                                else if(in_array($proses,$ario_array)){
                                    if(!in_array($noDemand,$array_nodemand)){
                                        $data_all['ario']['JUMLAHKK']=floatval($data_all['ario']['JUMLAHKK'])+1;
                                        $array_nodemand[]=$noDemand;
                                    }
                                    $data_all['ario']['TOTALQTY']=floatval($data_all['ario']['TOTALQTY'])+$qty;
                                }

                                else if(in_array($proses,$sisir_array)){
                                    if(!in_array($noDemand,$array_nodemand)){
                                        $data_all['sisir']['JUMLAHKK']=floatval($data_all['sisir']['JUMLAHKK'])+1;
                                        $array_nodemand[]=$noDemand;
                                    }
                                    $data_all['sisir']['TOTALQTY']=floatval($data_all['sisir']['TOTALQTY'])+$qty;
                                }
                                
                                else if(in_array($proses,$peach_array)){
                                    if(!in_array($noDemand,$array_nodemand)){
                                        $data_all['peach']['JUMLAHKK']=floatval($data_all['peach']['JUMLAHKK'])+1;
                                        $array_nodemand[]=$noDemand;
                                    }
                                    $data_all['peach']['TOTALQTY']=floatval($data_all['peach']['TOTALQTY'])+$qty;
                                }
                                
                                else if(in_array($proses,$peach_greige_array)){
                                    if(!in_array($noDemand,$array_nodemand)){
                                        $data_all['peach_greige']['JUMLAHKK']=floatval($data_all['peach_greige']['JUMLAHKK'])+1;
                                        $array_nodemand[]=$noDemand;
                                    }
                                    $data_all['peach_greige']['TOTALQTY']=floatval($data_all['peach_greige']['TOTALQTY'])+$qty;
                                }
                                else if(in_array($proses,$garuk_greige_array)){
                                    if(!in_array($noDemand,$array_nodemand)){
                                        $data_all['garuk_greige']['JUMLAHKK']=floatval($data_all['garuk_greige']['JUMLAHKK'])+1;
                                        $array_nodemand[]=$noDemand;
                                    }
                                    $data_all['garuk_greige']['TOTALQTY']=floatval($data_all['garuk_greige']['TOTALQTY'])+$qty;
                                }
                                else if(in_array($proses,$garuk_fleece_tambah_obat_array)){
                                    if(!in_array($noDemand,$array_nodemand)){
                                        $data_all['garuk_fleece_tambah_obat']['JUMLAHKK']=floatval($data_all['garuk_fleece_tambah_obat']['JUMLAHKK'])+1;
                                        $array_nodemand[]=$noDemand;
                                    }
                                    $data_all['garuk_fleece_tambah_obat']['TOTALQTY']=floatval($data_all['garuk_fleece_tambah_obat']['TOTALQTY'])+$qty;
                                }
                            }	
                            ?>
                            <td rowspan="2"><strong>GRK FLEECE</strong></td>
                            <td><strong><span style="text-align:center;">FIN 1X</span></strong></td>
							<td style="text-align:center;"><?= number_format(($data_all['grk_fleece_final']['JUMLAHKK']+$TjkGarukBantuFL ) ?? 0); ?></td>
                            <td style="text-align:center;"><?= number_format(($data_all['grk_fleece_final']['TOTALQTY']+$TqtyGarukBantuFL ) ?? 0, 2); ?></td>
                        </tr>
                        <tr>
                            <td><strong>FIN FINAL</strong></td>
                            <td style="text-align:center;"><?= number_format($rowFINAL['JUMLAHKK1'] ?? 0); ?></td>
                            <td style="text-align:center;"><?= number_format($rowFINAL['TOTAL_QTY1'] ?? 0, 2); ?></td>
                        </tr>
                        <tr>
                            <?php
...
-->
							...
							<td style="text-align:center;"><?= number_format(($data_all['grk_ap']['JUMLAHKK']+$TjkGarukBantuAP) ?? 0) ?></td>
                            <td style="text-align:center;"><?= number_format(($data_all['grk_ap']['TOTALQTY']+$TqtyGarukBantuAP) ?? 0, 2) ?></td>
                        </tr>
                        <tr>
                            <td colspan="2"><strong>POTONG BULU LAIN-LAIN</strong></td>
                            <td style="text-align:center;"><?= number_format($data_all['potongbulu_lainlain']['JUMLAHKK'] ?? 0); ?></td>
                            <td style="text-align:center;"><?= number_format($data_all['potongbulu_lainlain']['TOTALQTY'] ?? 0, 2); ?></td>
                        </tr>
                        <tr>
                            <td colspan="2"><strong>ANTI PILLING LAIN-LAIN</strong></td>
                            <td style="text-align:center;"><?= number_format($data_all['anti_pilling_lainlain']['JUMLAHKK'] ?? 0); ?></td>
                            <td style="text-align:center;"><?= number_format($data_all['anti_pilling_lainlain']['TOTALQTY'] ?? 0, 2); ?></td>
                        </tr>
                        <tr>
                            <td colspan="2"><strong>AIRO</strong></td>
                            <td style="text-align:center;"><?= number_format($data_all['ario']['JUMLAHKK'] ?? 0); ?></td>
                            <td style="text-align:center;"><?= number_format($data_all['ario']['TOTALQTY'] ?? 0, 2); ?></td>
                        </tr>
                        <tr>
                            <td colspan="2"><strong>SISIR</strong></td>
                            <td style="text-align:center;"><?= number_format($data_all['sisir']['JUMLAHKK'] ?? 0); ?></td>
                            <td style="text-align:center;"><?= number_format($data_all['sisir']['TOTALQTY'] ?? 0, 2); ?></td>
                        </tr>
                        <tr>
                            <td colspan="2"><strong>PEACH SKIN</strong></td>
                            <td style="text-align:center;"><?= number_format($data_all['peach']['JUMLAHKK'] ?? 0); ?></td>
                            <td style="text-align:center;"><?= number_format($data_all['peach']['TOTALQTY'] ?? 0, 2); ?></td>
                        </tr>
                        <tr>
                            <td colspan="2"><strong>PEACH SKIN GREIGE</strong></td>
                            <td style="text-align:center;"><?= number_format($data_all['peach_greige']['JUMLAHKK'] ?? 0); ?></td>
                            <td style="text-align:center;"><?= number_format($data_all['peach_greige']['TOTALQTY'] ?? 0, 2); ?></td>
                        </tr>
                        <tr>
                            <td colspan="2"><strong>GARUK GREIGE</strong></td>
                            <td style="text-align:center;"><?= number_format($data_all['garuk_greige']['JUMLAHKK'] ?? 0); ?></td>
                            <td style="text-align:center;"><?= number_format($data_all['garuk_greige']['TOTALQTY'] ?? 0, 2); ?></td>
                        </tr>
                        <tr>
                            <td colspan="2"><strong>GARUK FLEECE TAMBAH OBAT</strong></td>
                            <td style="text-align:center;"><?= number_format($data_all['garuk_fleece_tambah_obat']['JUMLAHKK'] ?? 0); ?></td>
                            <td style="text-align:center;"><?= number_format($data_all['garuk_fleece_tambah_obat']['TOTALQTY'] ?? 0, 2); ?></td>
                        </tr>
                        <tr style="font-weight:bold; background-color:yellow;">
                            <?php
                            // Hitung total JUMLAHKK dan TOTAL_QTY dari semua proses di atas (kecuali PERBAIKAN)
...
//                                ((int)($rowAPf['JUMLAHKK'] ?? 0)) +
								((int)($TjkGarukBantuFL ?? 0)) +
								((int)($TjkGarukBantuAP ?? 0)) +
								((int)($data_all['grk_fleece_final']['JUMLAHKK'] ?? 0)) +
								((int)($data_all['grk_ap']['JUMLAHKK'] ?? 0)) +
                                ((int)($data_all['potongbulu_lainlain']['JUMLAHKK'] ?? 0)) +
                                ((int)($data_all['anti_pilling_lainlain']['JUMLAHKK'] ?? 0)) +
                                ((int)($data_all['ario']['JUMLAHKK'] ?? 0)) +
                                ((int)($data_all['sisir']['JUMLAHKK'] ?? 0)) +
                                ((int)($data_all['peach']['JUMLAHKK'] ?? 0)) +
                                ((int)($data_all['peach_greige']['JUMLAHKK'] ?? 0)) +
                                ((int)($data_all['garuk_greige']['JUMLAHKK'] ?? 0)) +
                            	((int)($data_all['garuk_fleece_tambah_obat']['JUMLAHKK'] ?? 0));
...
//                                ((float)($rowAPf['TOTAL_QTY'] ?? 0))+
								((float)($TqtyGarukBantuFL ?? 0)) +
								((float)($TqtyGarukBantuAP ?? 0)) +
								((float)($data_all['grk_fleece_final']['TOTALQTY'] ?? 0)) +
								((float)($data_all['grk_ap']['TOTALQTY'] ?? 0)) +
                                ((float)($data_all['potongbulu_lainlain']['TOTALQTY'] ?? 0)) +
                                ((float)($data_all['anti_pilling_lainlain']['TOTALQTY'] ?? 0)) +
                                ((float)($data_all['ario']['TOTALQTY'] ?? 0)) +
                                ((float)($data_all['sisir']['TOTALQTY'] ?? 0)) +
                                ((float)($data_all['peach']['TOTALQTY'] ?? 0)) +
                                ((float)($data_all['peach_greige']['TOTALQTY'] ?? 0)) +
                                ((float)($data_all['garuk_greige']['TOTALQTY'] ?? 0)) +
                                ((float)($data_all['garuk_fleece_tambah_obat']['TOTALQTY'] ?? 0))
                                ;
                            ?>
...
<!--            </tr>-->
      ...
      <td width="30%" align="left" valign="top">
		...
		<table border="1" class="table-list1" width="100%">
            ...
            <?php 
            $sql = "SELECT *
                    FROM history_cari_gerobak_otomatis
                    WHERE TGL = '".$tanggalAkhir_tbl4->format('Y-m-d')."' ";

            $result = mysqli_query($conb, $sql);

            $wet_suiding_array=['WET1', 'WET2', 'WET3', 'WET4'];
            $airo_array=['AIR1'];
            $anti_pilling_array=['TDR1'];
            $sisir_array=['COM1', 'COM2', 'WET3', 'WET4'];
            $garuk_array=['RSE1', 'RSE2', 'RSE3', 'RSE4', 'RSE5'];
            $potong_bulu_array=['SHR1', 'SHR2', 'SHR3', 'SHR4', 'SHR5'];
            $peach_skin_array=['SUE1', 'SUE2', 'SUE3', 'SUE4'];
            $polishing_array=['POL1'];
            $adm_brs_array=['WAIT38'];
            $inspek_brs_array=['INS9'];
            $ncp_array=['NCP8'];
            $persiapan_kosong_array=[];
            
            $data_all=array();
            $array_nodemand=array();
            $array_qty_gerobak_jml=array();
            //operation, depatement, gerobak, qty , jml gerobak
            if(!$result){
                echo mysqli_error($conb);
            }
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    if ($row['DEPARTEMEN'] == "BRS") {
                        $noDemand=trim($row['DEMAND']," ");
                        $operation=$row['OPERATION'];
                        $gerobak=$row['GEROBAK'];
                        $qty=$row['QTY'];
                        $jml=$row['JML_GEROBAK'];

                        $qty_gerobak_jml=$qty."".$gerobak."".$jml;
                        if(intval($jml) !="0"){
                            if(in_array($qty_gerobak_jml,$array_qty_gerobak_jml)){
                                continue;
                            }
                        }

                        $array_qty_gerobak_jml[]=$qty_gerobak_jml;

                        if(in_array($operation,$wet_suiding_array)){
                            if(!in_array($noDemand,$array_nodemand)){
                                $data_all['WET']['JUMLAHKK']=floatval($data_all['WET']['JUMLAHKK'])+1;
                                $array_nodemand[]=$noDemand;
                            }
                            $data_all['WET']['TOTALQTY']=floatval($data_all['WET']['TOTALQTY'])+$qty;
                        }
                        else if(in_array($operation,$airo_array)){
                            if(!in_array($noDemand,$array_nodemand)){
                                $data_all['AIR']['JUMLAHKK']=floatval($data_all['AIR']['JUMLAHKK'])+1;
                                $array_nodemand[]=$noDemand;
                            }
                            $data_all['AIR']['TOTALQTY']=floatval($data_all['AIR']['TOTALQTY'])+$qty;
                        }
                        else if(in_array($operation,$anti_pilling_array)){
                            if(!in_array($noDemand,$array_nodemand)){
                                $data_all['PILLING']['JUMLAHKK']=floatval($data_all['PILLING']['JUMLAHKK'])+1;
                                $array_nodemand[]=$noDemand;
                            }
                            $data_all['PILLING']['TOTALQTY']=floatval($data_all['PILLING']['TOTALQTY'])+$qty;
                        }
                        else if(in_array($operation,$sisir_array)){
                            if(!in_array($noDemand,$array_nodemand)){
                                $data_all['SISIR']['JUMLAHKK']=floatval($data_all['SISIR']['JUMLAHKK'])+1;
                                $array_nodemand[]=$noDemand;
                            }
                        ...
                        else if(in_array($operation,$garuk_array)){
                            if(!in_array($noDemand,$array_nodemand)){
                                $data_all['GARUK']['JUMLAHKK']=floatval($data_all['GARUK']['JUMLAHKK'])+1;
                                $array_nodemand[]=$noDemand;
                            }
                            $data_all['GARUK']['TOTALQTY']=floatval($data_all['GARUK']['TOTALQTY'])+$qty;
                        }
                        else if(in_array($operation,$potong_bulu_array)){
                            if(!in_array($noDemand,$array_nodemand)){
                                $data_all['BULU']['JUMLAHKK']=floatval($data_all['BULU']['JUMLAHKK'])+1;
                                $array_nodemand[]=$noDemand;
                            }
                            $data_all['BULU']['TOTALQTY']=floatval($data_all['BULU']['TOTALQTY'])+$qty;
                        }
                        else if(in_array($operation,$peach_skin_array)){
                            if(!in_array($noDemand,$array_nodemand)){
                                $data_all['PEACH']['JUMLAHKK']=floatval($data_all['PEACH']['JUMLAHKK'])+1;
                                $array_nodemand[]=$noDemand;
                            }
                        ...
                        else if(in_array($operation,$polishing_array)){
                            if(!in_array($noDemand,$array_nodemand)){
                                $data_all['POLISH']['JUMLAHKK']=floatval($data_all['POLISH']['JUMLAHKK'])+1;
                                $array_nodemand[]=$noDemand;
                            }
                            $data_all['POLISH']['TOTALQTY']=floatval($data_all['POLISH']['TOTALQTY'])+$qty;
                        }
                        else if(in_array($operation,$adm_brs_array)){
                            if(!in_array($noDemand,$array_nodemand)){
                                $data_all['ADM']['JUMLAHKK']=floatval($data_all['ADM']['JUMLAHKK'])+1;
                                $array_nodemand[]=$noDemand;
                            }
                            $data_all['ADM']['TOTALQTY']=floatval($data_all['ADM']['TOTALQTY'])+$qty;
                        }
                        else if(in_array($operation,$inspek_brs_array)){
                            if(!in_array($noDemand,$array_nodemand)){
                                $data_all['INSPEK']['JUMLAHKK']=floatval($data_all['INSPEK']['JUMLAHKK'])+1;
                                $array_nodemand[]=$noDemand;
                            }
                            $data_all['INSPEK']['TOTALQTY']=floatval($data_all['INSPEK']['TOTALQTY'])+$qty;
                        }
                        else if(in_array($operation,$ncp_array)){
                            if(!in_array($noDemand,$array_nodemand)){
                                $data_all['NCP']['JUMLAHKK']=floatval($data_all['NCP']['JUMLAHKK'])+1;
                                $array_nodemand[]=$noDemand;
                            }
                            $data_all['NCP']['TOTALQTY']=floatval($data_all['NCP']['TOTALQTY'])+$qty;
                        }
                        else if(in_array($operation,$persiapan_kosong_array)){
                            if(!in_array($noDemand,$array_nodemand)){
                                $data_all['KOSONG']['JUMLAHKK']=floatval($data_all['KOSONG']['JUMLAHKK'])+1;
                                $array_nodemand[]=$noDemand;
                            }
                            $data_all['KOSONG']['TOTALQTY']=floatval($data_all['KOSONG']['TOTALQTY'])+$qty;
                        }
            ...
            <tr>
                ...
                <td style="text-align:center;"><?= number_format($data_all['WET']['JUMLAHKK'] ?? 0); ?></td>
                <td style="text-align:center;"><?= number_format($data_all['WET']['TOTALQTY'] ?? 0, 2); ?></td>
            </tr>
            <tr>
                <td ><strong>AIRO</strong></td>
                <td style="text-align:center;"><?= number_format($data_all['AIR']['JUMLAHKK'] ?? 0); ?></td>
                <td style="text-align:center;"><?= number_format($data_all['AIR']['TOTALQTY'] ?? 0, 2); ?></td>
            </tr>
            <tr>
                <td ><strong>ANTI PILLING</strong></td>
                <td style="text-align:center;"><?= number_format($data_all['PILLING']['JUMLAHKK'] ?? 0); ?></td>
                <td style="text-align:center;"><?= number_format($data_all['PILLING']['TOTALQTY'] ?? 0, 2); ?></td>
            </tr>
            <tr>
                <td ><strong>SISIR</strong></td>
                <td style="text-align:center;"><?= number_format($data_all['SISIR']['JUMLAHKK'] ?? 0); ?></td>
            ...
            <tr>
                <td ><strong>GARUK</strong></td>
                <td style="text-align:center;"><?= number_format($data_all['GARUK']['JUMLAHKK'] ?? 0); ?></td>
                <td style="text-align:center;"><?= number_format($data_all['GARUK']['TOTALQTY'] ?? 0, 2); ?></td>
            </tr>
            <tr>
                <td ><strong>POTONG BULU</strong></td>
                <td style="text-align:center;"><?= number_format($data_all['BULU']['JUMLAHKK'] ?? 0); ?></td>
                <td style="text-align:center;"><?= number_format($data_all['BULU']['TOTALQTY'] ?? 0, 2); ?></td>
            </tr>
            <tr>
                <td ><strong>PEACH SKIN</strong></td>
                <td style="text-align:center;"><?= number_format($data_all['PEACH']['JUMLAHKK'] ?? 0); ?></td>
            ...
            <tr>
                ...
                <td style="text-align:center;"><?= number_format($data_all['POLISH']['JUMLAHKK'] ?? 0); ?></td>
                <td style="text-align:center;"><?= number_format($data_all['POLISH']['TOTALQTY'] ?? 0, 2); ?></td>
            </tr>
            <tr>
                <td ><strong>ADM BRS</strong></td>
                <td style="text-align:center;"><?= number_format($data_all['ADM']['JUMLAHKK'] ?? 0); ?></td>
                <td style="text-align:center;"><?= number_format($data_all['ADM']['TOTALQTY'] ?? 0, 2); ?></td>
            </tr>
            <tr>
                <td ><strong>INSPEK BRS</strong></td>
                <td style="text-align:center;"><?= number_format($data_all['INSPEK']['JUMLAHKK'] ?? 0); ?></td>
                <td style="text-align:center;"><?= number_format($data_all['INSPEK']['TOTALQTY'] ?? 0, 2); ?></td>
            </tr>
            <tr>
                <td ><strong>NCP</strong></td>
                <td style="text-align:center;"><?= number_format($data_all['NCP']['JUMLAHKK'] ?? 0); ?></td>
                <td style="text-align:center;"><?= number_format($data_all['NCP']['TOTALQTY'] ?? 0, 2); ?></td>
            </tr>
            <tr>
                <td ><strong>PERSIAPAN / KOSONG</strong></td>
                <td style="text-align:center;"><?= number_format($data_all['KOSONG']['JUMLAHKK'] ?? 0); ?></td>
                <td style="text-align:center;"><?= number_format($data_all['KOSONG']['TOTALQTY'] ?? 0, 2); ?></td>
            </tr>
            ...
            <tr style="font-weight:bold; background-color:yellow;">
                ...
                $total_jumlahkkk=0;
                $total_qtyy=0;
                foreach($data_all as $id => $vd){
                    $total_jumlahkkk += intval($vd['JUMLAHKK']);
                    $total_qtyy += floatval($vd['TOTALQTY']);
                }
                ?>
                    			...
                    			FROM (
								 SELECT
									...
									WHERE										
										TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) >= TIMESTAMP('$tglAwal_tbl4')
                                        AND TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) <= TIMESTAMP('$tglAkhir_tbl4')
										AND pd.DLVSALORDLINESALORDCNTCODE IN ('DOMESTIC', 'EXPORT', 'OPEN')
										AND p.OPERATIONCODE IN (
		                                    'AIR1', 'COM1', 'COM2', 'POL1', 'RSE1', 'RSE2', 'RSE3', 'RSE4', 'RSE5',
		                                    'SHR1', 'SHR2', 'SHR3', 'SHR4', 'SHR5', 'SUE1', 'SUE2', 'SUE3', 'SUE4',
		                                    'TDR1', 'WET1', 'WET2', 'WET3', 'WET4'
              ...
              <?php
                ...
                $query = "select
                            ...
                            with ranked_data as (
                                ...
                                order by
                                    tgl_buat desc) as rn,
                                   CASE
							    WHEN tgl_proses_in = '0000-00-00' AND tgl_proses_out = '0000-00-00' THEN
							        (TIMESTAMPDIFF(HOUR,
							            STR_TO_DATE(jam_in, '%H:%i:%s'),
							            STR_TO_DATE(jam_out, '%H:%i:%s')
							        ))
							    WHEN tgl_proses_in = '0000-00-00' OR tgl_proses_out = '0000-00-00' THEN
							        (TIMESTAMPDIFF(HOUR,
							            STR_TO_DATE(jam_in, '%H:%i:%s'),
							            STR_TO_DATE(jam_out, '%H:%i:%s')
							        )) * (-1)   
							    ELSE
							        (TIMESTAMPDIFF(HOUR,
							            STR_TO_DATE(CONCAT(tgl_proses_in, ' ', jam_in), '%Y-%m-%d %H:%i:%s'),
							            STR_TO_DATE(CONCAT(tgl_proses_out, ' ', jam_out), '%Y-%m-%d %H:%i:%s')
							        ))
								END AS durasi_jam
                                from
                                    tbl_produksi
                            ...
                            where
                                rn = 1
                                and (durasi_jam between 0 and 30 or durasi_jam is null)) as t
                            ";
                $result = mysqli_query($conb, $query);
                ...
                $query_tidak_tercapai_30_jam_libur =
                    ...
                    (
                        ...
                        where
                            rn = 1
                            and (durasi_jam > 30 or durasi_jam < 0)
                    			...
                    			FROM (
								 SELECT
									...
									WHERE										
										TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) >= TIMESTAMP('$tglAwal_tbl4')
                                        AND TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) <= TIMESTAMP('$tglAkhir_tbl4')
										AND pd.DLVSALORDLINESALORDCNTCODE IN ('REPLCEXP','REPLACE')
										AND p.OPERATIONCODE IN (
		                                    'AIR1', 'COM1', 'COM2', 'POL1', 'RSE1', 'RSE2', 'RSE3', 'RSE4', 'RSE5',
		                                    'SHR1', 'SHR2', 'SHR3', 'SHR4', 'SHR5', 'SUE1', 'SUE2', 'SUE3', 'SUE4',
		                                    'TDR1', 'WET1', 'WET2', 'WET3', 'WET4'
                ...
                $query = "select
                            ...
                            with ranked_data as (
                                ...
                                where
                                    tgl_buat >= '$tglAwal_tbl4'
                                    and tgl_buat <= '$tglAkhir_tbl4'
                                    and (no_order like 'REP%' or no_order like 'RPE%')
                            )
                            select
                                *
                            from
                            ...
                            where
                                rn = 1
                                and (durasi_jam between 0 and 12 or durasi_jam is null)) as t";
                    			...
                    			FROM (
								 SELECT
									...
									WHERE										
										TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) >= TIMESTAMP('$tglAwal_tbl4')
                                        AND TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) <= TIMESTAMP('$tglAkhir_tbl4')
										AND pd.DLVSALORDLINESALORDCNTCODE IN ('MNB','MBE')
										AND p.OPERATIONCODE IN (
		                                    'AIR1', 'COM1', 'COM2', 'POL1', 'RSE1', 'RSE2', 'RSE3', 'RSE4', 'RSE5',
		                                    'SHR1', 'SHR2', 'SHR3', 'SHR4', 'SHR5', 'SUE1', 'SUE2', 'SUE3', 'SUE4',
		                                    'TDR1', 'WET1', 'WET2', 'WET3', 'WET4'
                ...
                $query = "select
                            ...
                            with ranked_data as (
                                ...
                                where
                                    tgl_buat >= '$tglAwal_tbl4'
                                    and tgl_buat <= '$tglAkhir_tbl4'
                                    and (no_order like 'MBE%' or no_order like 'MNB%' or no_order like 'DMB%')
                            )
                            select
                                *
                            from
...
//                ";
                    			...
                    			FROM (
								 SELECT
									...
									WHERE										
										TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) >= TIMESTAMP('$tglAwal_tbl4')
                                        AND TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) <= TIMESTAMP('$tglAkhir_tbl4')
										AND pd.DLVSALORDLINESALORDCNTCODE IN ('SAMPDOM', 'SAMPLE')
										AND p.OPERATIONCODE IN (
		                                    'AIR1', 'COM1', 'COM2', 'POL1', 'RSE1', 'RSE2', 'RSE3', 'RSE4', 'RSE5',
		                                    'SHR1', 'SHR2', 'SHR3', 'SHR4', 'SHR5', 'SUE1', 'SUE2', 'SUE3', 'SUE4',
		                                    'TDR1', 'WET1', 'WET2', 'WET3', 'WET4'
              ...
              <?php
                $query_SALESMAN_SAMPLE_12_JAM =
                    "select
                                COUNT(*) as TOTAL_SALESMAN_SAMPLE_12_JAM
                            from
                                (
                            with ranked_data as (
                                select
                    			...
                    			FROM (
								 SELECT
									...
									WHERE										
										TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) >= TIMESTAMP('$tglAwal_tbl4')
                                        AND TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) <= TIMESTAMP('$tglAkhir_tbl4')
										AND ( pd.DLVSALORDLINESALORDCNTCODE IN (
											'DEVBP', 'DEVINDGA', 'DEVINDGB', 'DEVINDGC', 'DEVINDGD', 'DEVINDMA', 'DEVINDMB', 
											'DEVINDMC', 'DEVINDMD', 'DEVINDME', 'DEVINDMF', 'DEVINDMG', 'DEVINDMH', 'DEVINDMI', 
											'DEVINDTA', 'DEVINDTB', 'TAS', 'TR', 'TS', 'S'
										) OR NOT pd.PROJECTCODE IS NULL)
										AND p.OPERATIONCODE IN (
		                                    'AIR1', 'COM1', 'COM2', 'POL1', 'RSE1', 'RSE2', 'RSE3', 'RSE4', 'RSE5',
		                                    'SHR1', 'SHR2', 'SHR3', 'SHR4', 'SHR5', 'SUE1', 'SUE2', 'SUE3', 'SUE4',
		                                    'TDR1', 'WET1', 'WET2', 'WET3', 'WET4'
                ...
                $query = "select
                            ...
                            with ranked_data as (
                                ...
                                where
                                    tgl_buat >= '$tglAwal_tbl4'
                                    and tgl_buat <= '$tglAkhir_tbl4'
                                    and (no_order like 'BP%' OR no_order like 'TS%' OR no_order like 'S%' OR no_order like 'TR%' OR no_order like 'DG%' OR no_order like 'DT%')
									and no_order NOT LIKE 'SME%' 
									and no_order NOT LIKE 'SAM%'
                            )
                            select
                                *
                            from
                    			...
                    			FROM (
								 SELECT
									...
									WHERE										
										TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) >= TIMESTAMP('$tglAwal_tbl4')
                                        AND TIMESTAMP(p.PROGRESSSTARTPROCESSDATE, p.PROGRESSSTARTPROCESSTIME) <= TIMESTAMP('$tglAkhir_tbl4')
										AND pd.DLVSALORDLINESALORDCNTCODE IN ('RETRNFAB','RETRNEXP')
										AND p.OPERATIONCODE IN (
		                                    'AIR1', 'COM1', 'COM2', 'POL1', 'RSE1', 'RSE2', 'RSE3', 'RSE4', 'RSE5',
		                                    'SHR1', 'SHR2', 'SHR3', 'SHR4', 'SHR5', 'SUE1', 'SUE2', 'SUE3', 'SUE4',
		                                    'TDR1', 'WET1', 'WET2', 'WET3', 'WET4'
