<script type="text/javascript">
    ...
    function proses_demand() {
        ...
        if (nodemand == 0) {
            window.location.href = '?p=Input-Lap-Jahit-New';
        } else {
            window.location.href = '?p=Input-Lap-Jahit-New&nodemand=' + nodemand;
        }
    }
    ...
    function proses_shift() {
        ...
        if (nodemand == "") {
            ...
            window.location.href = '?p=Input-Lap-Jahit-New&nodemand=' + nodemand + '&shift=' + shift;
        }
    }
</script>
<?php
    ...
    if ($_POST['simpan'] == "simpan") {
        ...
        $ceksql = sqlsrv_query($cond, "
            SELECT TOP 1 *
            FROM db_qc.tbl_jahit
            WHERE nodemand = '$_GET[nodemand]'
            AND [shift]  = '$_POST[shift]'
            AND CAST(tgl_jahit AS date) = CAST(GETDATE() AS date)
        ");

        if ($ceksql === false) {
            echo "<pre>";
            echo "=== SQLSRV ERROR (ceksql) ===\n\n";
        ...
        if ($cek > 0) {
            ...
            if ($sql1) {
                echo "<script>swal({
                    ...
                    if (result.value) {
                        window.location.href='?p=Input-Lap-Jahit-New&nodemand=$_POST[nodemand]';
                    }
                });</script>";
            }
        } else {
            ...
            $sql = sqlsrv_query($cond, "
                ...
                (
                    nokk, nodemand, no_order, no_po, langganan, jenis_kain, jenis_kain_body, warna,
                    lot, lot_body, roll, bruto, status, operator, tgl_fin, tgl_jahit, [shift],
                    ket, disposisi, colorist_qcf, tgl_buat, tgl_update
                )
                VALUES
                (
                    '$_POST[nokk]','$_POST[nodemand]','$order','$po','$langganan','$jns','$jns_body','$warna',
                    '$_POST[lot]','$_POST[lot_body]','$_POST[roll]','$_POST[bruto]','$_POST[status]','$_POST[operator]',
                    '$_POST[tgl_fin]','$_POST[tgl_jahit]','$_POST[shift]',
                    '$ket','$_POST[disposisi]','$colorist_qcf',GETDATE(),GETDATE()
                )
            ");

            if ($sql === false) {
                echo "<pre>";
                echo "=== SQLSRV ERROR (INSERT) ===\n\n";
                print_r(sqlsrv_errors());
                echo "</pre>";
                die;
            }
...
$msql = sqlsrv_query($cond, "SELECT * FROM db_qc.tbl_jahit WHERE nodemand='$nodemand' and [shift]='$_GET[shift]' AND CAST(tgl_jahit AS date)=CAST(GETDATE() AS date)");
...
<form class="form-horizontal" action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
    <div class="box box-info">
        ...
        <div class="box-body">
            <div class="col-md-6">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <textarea name="jenis_kain" class="form-control" id="jenis_kain" placeholder="Jenis Kain Accessories"><?php if ($crow > 0) {
                                                                                                                                    echo $row['jenis_kain'];
                                                                                                                                } else {
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <textarea name="jenis_kain_body" class="form-control" id="jenis_kain_body" placeholder="Jenis Kain Body"><?php if ($crow > 0) {
                                                                                                                                        echo $row['jenis_kain_body'];
                                                                                                                                    } ?></textarea>
                    </div>
                </div>
                <div class="form-group">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        <div class="input-group date">
                            ...
                            <input name="tgl_jahit" type="text" class="form-control pull-right" id="datepicker1" placeholder="0000-00-00" value="<?php if ($crow > 0) {
                                                                                                                                                        echo $row['tgl_jahit'];
                                                                                                                                                    } ?>" required />
                        </div>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-5">
                        <input name="tgl_fin" class="form-control" type="text" id="tgl_fin" value="<?php if ($crow > 0) {
                                                                                                        echo $row['tgl_fin'];
                                                                                                    } ?>" placeholder="Tgl Fin" required>
                    </div>
                </div>
                <!--<div class="form-group">		  	
                <label for="tgl_fin" class="col-sm-3 control-label">Tgl Fin</label>
                    <div class="col-sm-4">					  
                        <div class="input-group date">
                            <div class="input-group-addon"> <i class="fa fa-calendar"></i> </div>
                                <input name="tgl_fin" type="text" class="form-control pull-right" id="datepicker" placeholder="0000-00-00" value="<?php if ($crow > 0) {
                                                                                                                                                        echo $row['tgl_fin'];
                                                                                                                                                    } ?>" required/>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-3">
                        <div class="input-group">
                            <input name="bruto" type="text" class="form-control" id="bruto" value="<?php if ($crow > 0) {
                                                                                                        ...
                                                                                                        echo number_format($rowdb2['QTY_BRUTO'], '2', '.', '');
                    ...
                    <div class="col-sm-4">
                        <div class="input-group">
                            <select class="form-control select2" name="operator" id="operator" required>
                                ...
                                <?php
                                $qryp = sqlsrv_query($cond, "SELECT nama FROM db_qc.tbl_operator_jahit ORDER BY nama ASC");
                                while ($rp = sqlsrv_fetch_array($qryp, SQLSRV_FETCH_ASSOC)) {
                                ?>
                                    <option value="<?php echo $rp['nama']; ?>" <?php if ($row['operator'] == $rp['nama']) {
                                                                                    echo "SELECTED";
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-3">
                        <select class="form-control select2" name="status" onChange="tampil();">
                            ...
                            <option value="BEDA WARNA" <?php if ($row['status'] == "BEDA WARNA") {
                                                            echo "SELECTED";
                                                        } ?>>BEDA WARNA</option>
                            <option value="BELUM OK" <?php if ($row['status'] == "BELUM OK") {
                                                            echo "SELECTED";
                                                        } ?>>BELUM OK</option>
                        </select>
                    </div>
        ...
        <div class="box-footer">
            ...
            <button type="button" class="btn btn-warning pull-left" name="lihat" value="lihat" onClick="window.location.href='?p=Lihat-Data-Jahit-New'"><i class="fa fa-search"></i> Lihat Data</button>
        </div>
    </div>
</form>
...
if ($_POST['simpan_operator'] == "Simpan") {
    ...
    $sqlData1 = sqlsrv_query($cond, "INSERT INTO db_qc.tbl_operator_jahit SET 
		  nama='$nama'");
    if ($sqlData1) {
        echo "<script>swal({
