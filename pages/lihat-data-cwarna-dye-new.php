        ...
        <div class="box-body">
          <table class="table table-bordered table-hover table-striped nowrap" id="example1" style="width:100%">
            ...
            <tbody>
              ...
              if ($GShift != "ALL") {
                $shft = " AND t.[shift] ='$GShift' ";
              } else {
                $shft = " ";
              }

              if ($Awal != "" and $Akhir != "") {
                $where = " AND CAST(t.tgl_celup AS date) BETWEEN '$Awal' AND '$Akhir' ";
              } else {
                $where = " ";
              }
              ...
              if ($Awal != "" and $Akhir != "") {
                ...
                WHERE t.dept='QCF' $where $shft
                ORDER BY t.id ASC";
                      $qry1 = sqlsrv_query($cond, $sql);
                    } else {
                      $sql = "SELECT
                  t.*,
                  p.pemberi_instruksi
              ...
              ?>
                <tr bgcolor="<?php echo $bgcolor; ?>">
                  ...
                  <td align="center">
                    <div class="btn-group">
                      <a href="#" class="btn btn-danger btn-xs <?php if ($_SESSION['akses10'] == 'biasa' and ($_SESSION['lvl_id10'] != 'PACKING' or $_SESSION['lvl_id10'] != 'NCP')) {
                        ...
                        onclick="confirm_delete('./HapusDataCWarnaDyeNew-<?php echo $row1['id'] ?>');"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="Hapus"></i></a>
                    </div>
                  </td>

                  <td align="center">
                    <a data-type="date" data-pk="<?php echo $row1['id'] ?>" data-value="<?php echo ($row1['tgl_celup'] instanceof DateTime ? $row1['tgl_celup']->format('Y-m-d') : $row1['tgl_celup']); ?>"
                      class="tgl_celup" href="javascipt:void(0)">
                      <?php echo ($row1['tgl_celup'] instanceof DateTime ? $row1['tgl_celup']->format('Y-m-d') : $row1['tgl_celup']); ?>
                    </a>
                  </td>
                  ...
                  <td align="center"><a data-pk="<?php echo $row1['id'] ?>" data-value="<?php echo $row1['status_warna'] ?>" class="sts_warna" href="javascipt:void(0)"><?php echo $row1['status_warna'] ?></a></td>
                  ...
                  <td align="center">
                    <a data-pk="<?php echo $row1['id']; ?>"
                      data-value="<?php echo ($row1['spectro'] === null ? '' : $row1['spectro']); ?>"
                      class="spectro_fin" href="javascript:void(0)">
                      <?php echo ($row1['spectro'] === null ? '' : ($row1['spectro'] == 1 ? '✔' : '✖')); ?>
                    </a>
                  </td>
                </tr>
              <?php
