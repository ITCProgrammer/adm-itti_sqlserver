          ...
          <div class="box-body">
            <table class="table table-bordered table-hover table-striped nowrap" id="example3" style="width:100%">
              ...
              <tbody>
                ...
                $no = 1;
                if ($Awal != "" and $Dept != "") {
                  $Where = " WHERE CONVERT(date, tgl_buat) BETWEEN '$Awal' AND '$Akhir' AND (dept= '$Dept' OR t_jawab='$Dept' OR t_jawab1='$Dept' OR t_jawab2='$Dept' OR t_jawab3='$Dept' OR t_jawab4='$Dept') ";
                } else if ($Awal != "") {
                  $Where = " WHERE CONVERT(date, tgl_buat) BETWEEN '$Awal' AND '$Akhir' ";
                } else if ($Dept != "") {
                  $Where = " WHERE dept= '$Dept' OR t_jawab='$Dept' OR t_jawab1='$Dept' OR t_jawab2='$Dept' OR t_jawab3='$Dept' OR t_jawab4='$Dept'";
                } else if ($Order != "") {
                  $Where = " WHERE no_order= '$Order' ";
                } else if ($Hanger != "") {
                  $Where = " WHERE no_hanger= '$Hanger' ";
                } else if ($Masalah != "") {
                  $Where = " WHERE masalah LIKE '%$Masalah%' ";
                } else if ($Awal == "" and $Order == "" and $Hanger == "" and $Masalah == "" and $Dept == "") {
                  $Where = " WHERE CONVERT(date, tgl_buat) BETWEEN '$Awal' AND '$Akhir' ";
                }
                ...
                while ($row1 = sqlsrv_fetch_array($qry1, SQLSRV_FETCH_ASSOC)) {
                  $sqlgk = sqlsrv_query($cona, " SELECT * FROM db_adm.tbl_bonkain WHERE id_nsp=? ORDER BY no_bon ASC", [$row1['id']]);
                  ...
                  $data = explode(",", $dtArr);
                  if (in_array("Man", $data)) {
                    $sebab .= " <span class='label label-info'>Man</span> ";
                  }
                  if (in_array("Methode", $data)) {
                    $sebab .= " <span class='label label-warning'>Methode</span> ";
                  }
                  if (in_array("Machine", $data)) {
                    $sebab .= " <span class='label label-danger'>Machine</span> ";
                  }
                  if (in_array("Material", $data)) {
                    $sebab .= " <span class='label label-primary'>Material</span> ";
                  }
                  if (in_array("Environment", $data)) {
                    $sebab .= " <span class='label label-success'>Environment</span> ";
                  }
                ?>
                  <tr bgcolor="<?php echo $bgcolor; ?>">
                    <td align="center"><?php echo $no; ?></td>
                    <td align="center">
                      <div class="btn-group"><a href="index1.php?p=input-bon-kain&id=<?php echo $row1['id']; ?>" class="btn btn-warning btn-xs <?php if ($_SESSION['akses'] == 'biasa') {
                                                                                                                                                  echo "disabled";
                                                                                                                                                } ?>" target="_blank"><i class="fa fa-plus"></i> </a>
                        <a href="EditBon-<?php echo $row1['id']; ?>" class="btn btn-info btn-xs <?php if ($_SESSION['akses10'] == 'biasa') {
                                                                                                  echo "disabled";
                                                                                                } else {
                                                                                                  echo "disabled";
                                                                                                } ?>" target="_blank"><i class="fa fa-edit"></i> </a>
                        <a href="#" class="btn btn-danger btn-xs <?php if ($_SESSION['akses'] == 'biasa' or $rgk > 0) {
                                                                    echo "disabled";
                                                                  } ?>" onclick="confirm_delete('index1.php?p=hapusdatagantikain&id=<?php echo $row1['id']; ?>');"><i class="fa fa-trash"></i> </a>
                      </div>
                    </td>
                    <td align="center"><?php echo fmt_dt($row1['tgl_buat'] ?? null, 'Y-m-d'); ?></td>
                    ...
                    <td align="center"><?php echo fmt_dt($row1['tgl_delivery'] ?? null, 'Y-m-d'); ?></td>
                    ...
                    <?php if ($_SESSION['user_id10'] == "aressa.gasih" || $_SESSION['user_id10'] == "dit"): ?>
                      <td style="width: 100%;">
                        ...
                        if (!empty($row1['t_jawab'])) {
                          $sql = "SELECT nama FROM db_adm.tbl_dept WHERE nama = ?";

                          $params = [$row1['t_jawab']];

                          $stmt = sqlsrv_prepare($cona, $sql, $params);
                          if ($stmt === false) {
                    ...
                    <?php if ($_SESSION['user_id10'] == "aressa.gasih" || $_SESSION['user_id10'] == "dit"): ?>
                      <td style="width: 100%;">
                        ...
                        if (!empty($row1['t_jawab1'])) {
                          $query_nama = sqlsrv_prepare($cona, "SELECT nama FROM db_adm.tbl_dept WHERE nama = ?", [$row1['t_jawab1']]);
                          if ($query_nama && sqlsrv_execute($query_nama)) {
                            if ($row_nama = sqlsrv_fetch_array($query_nama, SQLSRV_FETCH_ASSOC)) {
                              $accresep2_nama = $row_nama['nama'];
                            }
                          }
                        }
                    ...
                    <?php if ($_SESSION['user_id10'] == "aressa.gasih" || $_SESSION['user_id10'] == "dit"): ?>
                      <td style="width: 100%;">
                        ...
                        if (!empty($row1['t_jawab2'])) {
                          $query_nama = sqlsrv_prepare($cona, "SELECT nama FROM db_adm.tbl_dept WHERE nama = ?", [$row1['t_jawab2']]);
                          if ($query_nama && sqlsrv_execute($query_nama)) {
                            if ($row_nama = sqlsrv_fetch_array($query_nama, SQLSRV_FETCH_ASSOC)) {
                              $accresep2_nama = $row_nama['nama'];
                            }
                    ...
                    <?php if ($_SESSION['user_id10'] == "aressa.gasih" || $_SESSION['user_id10'] == "dit"): ?>
                      <td style="width: 100%;">
                        ...
                        if (!empty($row1['t_jawab3'])) {
                          $query_nama = sqlsrv_prepare($cona, "SELECT nama FROM db_adm.tbl_dept WHERE nama = ?", [$row1['t_jawab3']]);
                          if ($query_nama && sqlsrv_execute($query_nama)) {
                            if ($row_nama = sqlsrv_fetch_array($query_nama, SQLSRV_FETCH_ASSOC)) {
                              $accresep2_nama = $row_nama['nama'];
                            }
                    ...
                    <?php if ($_SESSION['user_id10'] == "aressa.gasih" || $_SESSION['user_id10'] == "dit"): ?>
                      <td style="width: 100%;">
                        ...
                        if (!empty($row1['t_jawab4'])) {
                          $query_nama = sqlsrv_prepare($cona, "SELECT nama FROM db_adm.tbl_dept WHERE nama = ?", [$row1['t_jawab4']]);
                          if ($query_nama && sqlsrv_execute($query_nama)) {
                            if ($row_nama = sqlsrv_fetch_array($query_nama, SQLSRV_FETCH_ASSOC)) {
                              $accresep2_nama = $row_nama['nama'];
                            }
