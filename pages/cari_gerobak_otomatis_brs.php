<?php
    ...
    $check = mysqli_query($conb,"select id from history_cari_gerobak_otomatis 
            WHERE 
            TGL = '".date("Y-m-d")."' ") ;
    ...
    if($row_count!=0){
        echo "Cari Grobak BRS sudah di jalankan";
        exit;
    }

    function tmpData($demand, $operation, $departemen, $qty, $jml, $gerobak)
    {
        global $conb;

        if (! empty($operation) || ! empty($departemen)) {
            $sql = "INSERT INTO history_cari_gerobak_otomatis (DEMAND, OPERATION, DEPARTEMEN, QTY, JML_GEROBAK,GEROBAK,TGL)
                    VALUES ('$demand','$operation', '$departemen', $qty, $jml,'$gerobak','".date("Y-m-d")."')";
            mysqli_query($conb, $sql);
        }
    }

    $q_iptip = db2_exec($conn2, "SELECT * FROM
                                    (SELECT DISTINCT
                                        PRODUCTIONORDERCODE,
                                ...
                                WHERE NOT OPERATIONGROUPCODE = 'DYE' ");
   
    while ($row_iptip = db2_fetch_assoc($q_iptip)){
        // jika bukan Brushing di skip
        ...
        $q_posisikk = db2_exec($conn2, "SELECT DISTINCT
                                            ...
                                            LISTAGG(DISTINCT FLOOR(idqd.VALUEQUANTITY), ', ') AS GEROBAK
                                        FROM
                                            PRODUCTIONDEMANDSTEP p
                                        LEFT JOIN OPERATION o ON o.CODE = p.OPERATIONCODE
        ...
        if (! empty($row_posisikk['GEROBAK'])){            
            ...
            $demand_choice=$row_iptip['PRODUCTIONDEMANDCODE'];
            ...
            tmpData($demand_choice,$operation_choice, $dept_choice, $qty_choice, $jml_choice, $gerobak_choice);
        }
        else {
            // FIELDNYA = tempat, kain_gerobak
            $q_ncp   = mysqli_query($cond, "SELECT * FROM `tbl_ncp_qcf_now` WHERE nokk = '$row_iptip[PRODUCTIONORDERCODE]' AND nodemand = '$row_iptip[PRODUCTIONDEMANDCODE]'");
    ...
    echo "Berhasi menjalankan Cari Grobak BRS";
    exit;
