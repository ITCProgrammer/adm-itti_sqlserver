      ...
      <div class="modal-body">
        ...
        <table class="table table-bordered table-hover table-striped nowrap" id="example1" style="width: 100%; text-align: center;" border="1">
          <tbody>
            ...
            <?php
            $qCek = sqlsrv_query($cond, "SELECT tgl_buat,no_ncp_gabungan,masalah,masalah_dominan,nodemand
                            FROM db_qc.tbl_ncp_qcf_now
                            WHERE reg_no='$modal_id'");
            ...
            ?>
                <tr>
                  <td>
                    <?php
                    if (!empty($dCek['tgl_buat']) && $dCek['tgl_buat'] instanceof DateTime) {
                      echo $dCek['tgl_buat']->format('Y-m-d H:i:s');
                    } else {
                      echo $dCek['tgl_buat'];
                    }
                    ?>
